...
export default defineConfig({ base:'./',define:{__MB_BUILD_TAG__:JSON.stringify(buildTag())},plugins:[svelte()], server:{host:'127.0.0.1',fs:{allow:[searchForWorkspaceRoot('.'),new URL('../../packages/label-editor',import.meta.url).pathname,new URL(`${siblingDepth}mb-printer-sdk`,import.meta.url).pathname,new URL(`${siblingDepth}mb-ui`,import.meta.url).pathname]}}, build:{target:'es2022'} });
