...
<script lang="ts">
  ...
  import { AssetCatalogClient,BatchPanel,CloudPrintClient,CloudPrintPanel,CloudPrintRoute,DirectPrintPanel,JobRecoveryPanel,JobJournal,LabelEditor,LaPostePanel,LocalServicePanel,SheetPanel,createEditorStore,defaultDocument,downloadBytes,EditorDatabase,createAutosaver,openDocument,saveDocument,LocalApiPrintRoute,Modal,Menu,updateDocument,type CloudPrinter,type DocumentMaterializer,type LocalApiConnection,type PrinterSdk,type PrinterDefinition,type PrintProgress,type PrintRoute,type SheetExporter,type SheetFillOrder,type SheetLayoutPreset,type SheetPreferencesV1 } from '@makersbrain/label-editor'; import {loadPrinterSdk} from './sdk.js';
  const editor=createEditorStore(defaultDocument()); const database=new EditorDatabase(); let status='Ready'; const autosave=createAutosaver(database,1500,error=>status=`Autosave unavailable: ${message(error)}`); let sdk:(PrinterSdk&SheetExporter&DocumentMaterializer)|undefined;let printers:PrinterDefinition[]=[];let printerId='';let progress:PrintProgress|undefined;let apiToken=localStorage.getItem('mb-local-api-token')??'';let assetCatalogUrl=localStorage.getItem('mb-asset-catalog-url')??import.meta.env.VITE_ASSET_CATALOG_URL??'http://127.0.0.1:8766';let assetCatalogToken=localStorage.getItem('mb-asset-catalog-token')??'';let connectionId=localStorage.getItem('mb-local-api-connection')??'';let localConnection:LocalApiConnection|undefined;let cloudUrl=localStorage.getItem('mb-cloud-print-url')??'';let cloudTenant=localStorage.getItem('mb-cloud-print-tenant')??'';let cloudToken='';let cloudPrinterId=localStorage.getItem('mb-cloud-print-printer')??'';let cloudPrinter:CloudPrinter|undefined;let cloudClient:CloudPrintClient|undefined;let cloudRoute:CloudPrintRoute|undefined;let directRoute:PrintRoute|undefined;let handle:unknown;let online=navigator.onLine;let theme:'system'|'light'|'dark'='system';let defaultRoute='local-api';let sheet:SheetPreferencesV1|undefined;let updateAvailable=false;const localRoute=new LocalApiPrintRoute({token:()=>apiToken,connection:()=>localConnection,journal:new JobJournal(database)});let selectedRoute:PrintRoute=localRoute;
  let printing=false;
  ...
  onMount(()=>{let unsubscribe:(()=>void)|undefined;let disposed=false;const update=()=>updateAvailable=true;window.addEventListener('mb-pwa-update',update);void start().then(value=>{if(disposed)value();else unsubscribe=value});return()=>{disposed=true;unsubscribe?.();window.removeEventListener('mb-pwa-update',update);void autosave.dispose()}});
  let dialog='';
  ...
  async function restorePreferences(){try{const preferences=await database.getPreferences();if(preferences){editor.setView({gridSize:preferences.gridSize,showGrid:preferences.showGrid,showRulers:preferences.showRulers,snapping:preferences.snapping});printerId=preferences.defaultPrinterId??printerId;theme=preferences.theme;defaultRoute=preferences.defaultRoute??defaultRoute;sheet=preferences.sheet?.version===1?preferences.sheet:undefined;applyTheme()}}catch(error){status=`Preferences unavailable: ${message(error)}`}}
  function preferences(){return{gridSize:$editor.view.gridSize,showGrid:$editor.view.showGrid,showRulers:$editor.view.showRulers,snapping:$editor.view.snapping,defaultPrinterId:printerId||undefined,defaultRoute,theme,sheet}}function applyTheme(){if(theme==='system')delete document.documentElement.dataset.theme;else document.documentElement.dataset.theme=theme}function persistPreferences(){void database.savePreferences(preferences())}
  async function start(){await restorePreferences();await restoreAutosave();const unsubscribe=editor.subscribe(state=>{autosave(state.document);void database.savePreferences({...preferences(),gridSize:state.view.gridSize,showGrid:state.view.showGrid,showRulers:state.view.showRulers,snapping:state.view.snapping});if(state.document.template)void database.saveTemplate(state.document.id,state.document.template);void database.saveRecent({id:state.document.id,kind:'document',openedAt:new Date().toISOString()})});void ensureSdk().catch(error=>status=message(error));void restoreLocalConnection();return unsubscribe}
  ...
  async function ensureSdk(){if(!sdk){if(status==='Ready')status='Loading printer SDK…';sdk=await loadPrinterSdk(event=>{if(event.outcome==='failed')status=`Sheet ${event.operation} failed (${event.errorCode??'unknown'}).`;else if(event.operation==='export')status=`Sheet PDF built in ${Math.round(event.durationMs)} ms.`});printers=await sdk.printerDefinitions();printerId||=printers[0]?.id??'';}return sdk}
  ...
  function rememberSheet(layoutId:string,fillOrder:SheetFillOrder,custom?:NonNullable<SheetLayoutPreset['grid']>){sheet={version:1,layoutId,fillOrder,...(custom?{lastCustomGrid:custom}:{})};persistPreferences()}
...
<div class="app">
  ...
  <Modal open={dialog==='batch'} title="Batch printing" onClose={()=>dialog=''}>{#if selectedRoute.id==='cloud-api'&&!cloudPrinter?.online}<p class="pending">Cloud batch printing requires the selected printer to be online. Queue only one current label while it is offline.</p>{/if}{#if sdk}<BatchPanel document={$editor.document} {sdk} materializer={sdk} route={selectedRoute.id==='cloud-api'&&!cloudPrinter?.online?undefined:selectedRoute} printer={printers.find(item=>item.id===printerId)}/>{:else}<p class="pending">Select a printer model to load the printer SDK first.</p>{/if}</Modal>
  <Modal open={dialog==='sheet'} title="Label sheet" onClose={()=>dialog=''}>{#if sdk}<SheetPanel document={$editor.document} exporter={sdk} materializer={sdk} initialPreferences={sheet} onLayout={rememberSheet} onStatus={value=>status=value}/>{:else}<p class="pending">Loading sheet export support…</p>{/if}</Modal>
