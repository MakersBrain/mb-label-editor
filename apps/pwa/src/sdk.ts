...
import {
  ...
  assertDocumentReadyForOutput,
  defaultDocument,
  fromSdkDocument,
  isSheetPlan,
  isZoneBatchPlanForRequest,
  sdkPlanExecutor,
  structuredMaterializationError,
  toSdkDocument,
  uuid,
...
interface Stamp extends NormalizedPage {
  slot: number;
}
/** Identity of the embedded WebAssembly SDK, as reported by its `buildInfo()` export. */
export interface SdkBuildInfo {
  name: string;
  version: string;
  commit: string;
  dirty: boolean;
  protocolSourceCommit: string;
}
export type BrowserSdk = PrinterSdk &
  SheetExporter &
  DocumentMaterializer &
  DocumentMeasurer & { readonly buildInfo: SdkBuildInfo };
...
function adaptSdk(diagnostics?: (event: SheetDiagnostic) => void): BrowserSdk {
  ...
  const rememberStamp = (id: string, stamp: Stamp) => {
    const packed: Stamp = {
      ...stamp,
      pixels: stamp.pixels instanceof Uint8Array ? stamp.pixels : Uint8Array.from(stamp.pixels),
    };
    stampCache.delete(id);
    stampCache.set(id, packed);
    while (stampCache.size > STAMP_CACHE_LIMIT) stampCache.delete(stampCache.keys().next().value!);
    return packed;
  };
  const recallStamp = (id: string) => {
    const stamp = stampCache.get(id);
    if (stamp) {
      stampCache.delete(id);
  ...
  return {
    ...
    async materializeRecord(document, record, options) {
      ...
      try {
        return materializedDocument(
          JSON.parse(
            wasm.materializeRecord(
              JSON.stringify(toSdkDocument(document)),
              JSON.stringify(record),
              JSON.stringify(materializeOptions(options)),
            ),
          ) as unknown,
        );
      } catch (error) {
        throw structuredMaterializationError(error);
      }
    },
    async planZoneBatch(document, input) {
      try {
        const plan = JSON.parse(
          wasm.planZoneBatch(JSON.stringify(toSdkDocument(document)), JSON.stringify(input)),
        ) as unknown;
        if (!isZoneBatchPlanForRequest(plan, input)) throw new DocumentMaterializationError('request.encode_failed');
        return plan;
      } catch (error) {
        throw structuredMaterializationError(error);
      }
    ...
    async materializeZoneBatch(document, records, options) {
      try {
        const pages = JSON.parse(
          wasm.materializeZoneBatch(
            JSON.stringify(toSdkDocument(document)),
            JSON.stringify(records),
            JSON.stringify(materializeOptions(options)),
          ),
        ) as unknown;
        if (!Array.isArray(pages)) throw new DocumentMaterializationError('request.encode_failed');
        return pages.map(materializedDocument);
      } catch (error) {
        throw structuredMaterializationError(error);
      }
    },
    async validateCanonical(value) {
      const errors = JSON.parse(wasm.validateDocument(JSON.stringify(value))) as string[];
      return { valid: !errors.length, errors };
    },
    async importV3Canonical(value) {
      return JSON.parse(wasm.importV3(JSON.stringify(value))) as unknown;
    },
    async validate(document) {
      const errors = JSON.parse(wasm.validateDocument(JSON.stringify(toSdkDocument(document)))) as string[];
      return { valid: !errors.length, errors };
    },
    async render(document) {
      if (perf) perf.render++;
      assertDocumentReadyForOutput(document);
      return decodePng(wasm.renderPng(JSON.stringify(toSdkDocument(document))));
    },
    async exportPng(document) {
      assertDocumentReadyForOutput(document);
      return wasm.renderPng(JSON.stringify(toSdkDocument(document)));
    },
    async exportPdf(documents) {
      documents.forEach(assertDocumentReadyForOutput);
      return wasm.renderBatchPdf(JSON.stringify(documents.map(toSdkDocument)));
    },
    async planSheet(input, layout, options) {
      const started = performance.now();
      try {
        const plan = JSON.parse(
          wasm.planSheet(JSON.stringify(input), JSON.stringify(layout), JSON.stringify(options)),
        ) as unknown;
    ...
    async exportSheetPdf(documents, layout, options) {
      const started = performance.now();
      try {
        const data = wasm.buildSheetPdf(
          JSON.stringify(documents.map(toSdkDocument)),
    ...
    async plan(document, printer, options) {
      ...
      const request = JSON.stringify({
        ...
        ...(options.continuous
          ? {
              continuous: {
                cutMode: options.continuous.cutMode,
                extraFeedBeforeUm: Math.round(options.continuous.extraFeedBeforeMm * 1_000),
                extraFeedAfterUm: Math.round(options.continuous.extraFeedAfterMm * 1_000),
                chainCopies: options.continuous.chainCopies,
              },
            }
          : {}),
      });
      const parsed = JSON.parse(
        wasm.renderProtocolPlanWithOptions(JSON.stringify(toSdkDocument(document)), printer.id, request),
      ) as { protocol: string; source_commit?: string; actions: SdkPlanAction[] };
    ...
    async planBatch(documents, printer, options) {
      ...
      const parsed = JSON.parse(
        wasm.renderProtocolBatchPlanWithOptions(JSON.stringify(documents.map(toSdkDocument)), printer.id, request),
      ) as { protocol: string; source_commit?: string; actions: SdkPlanAction[] };
      return adaptSdkProtocolPlan(parsed.protocol, parsed.actions, parsed.source_commit);
    },
    ...
    async statusPlan(printer) {
      const parsed = JSON.parse(wasm.statusPlan(printer.id)) as {
        protocol: string;
        source_commit?: string;
        actions: SdkPlanAction[];
      };
      return adaptSdkProtocolPlan(parsed.protocol, parsed.actions, parsed.source_commit);
    },
    async parseStatus(printer: PrinterDefinition, frames: Uint8Array[]): Promise<PrinterStatus> {
      ...
      if (protocol === 'brother') {
        const status = JSON.parse(wasm.parseBrotherStatus(frames[frames.length - 1])) as Omit<
          PrinterStatus,
          'protocol' | 'raw'
        >;
        // Name the roll the printer just reported, so the reading is actionable.
        const media = JSON.parse(
          wasm.matchMedia(printer.id, status.mediaWidthMm ?? 0, status.mediaLengthMm ?? 0),
        ) as MediaPreset | null;
        return { protocol, ...status, media: media ?? undefined, raw: frames };
      }
      const status = JSON.parse(wasm.parsePhomemoStatus(JSON.stringify(frames.map((frame) => [...frame])))) as Omit<
        PrinterStatus,
        'protocol' | 'raw'
      >;
    ...
    async printerDefinitions() {
      const definitions = JSON.parse(wasm.printerCapabilities()) as {
        id: string;
        name: string;
        dpi: number;
        protocol: string;
        minRows?: number;
        maxRows?: number;
        continuousMedia?: PrinterDefinition['continuousMedia'];
      }[];
      return definitions.map((item) => {
        ...
        return {
          ...
          media: { minWidth: 1, maxWidth: 300, minHeight: minimumLength, maxHeight: maximumLength },
          ...(item.continuousMedia ? { continuousMedia: item.continuousMedia } : {}),
        };
      });
    },
    async importFirstPdfPage(data, dpi) {
      ...
      return {
        mimeType: 'image/png',
        data: await grayPng(page),
        widthMm: page.widthUm / 1000,
        heightMm: page.heightUm / 1000,
      };
    },
    async inspectLaPoste(data, format, options) {
      ...
      return stamps
        .filter((stamp) => !options?.pages || options.pages.includes(stamp.page))
        .map((stamp) => {
          const id = `${format}:${stamp.page}:${stamp.slot}`;
          const packed = rememberStamp(id, stamp);
          return {
            id,
            sourcePage: stamp.page,
            slot: stamp.slot,
            occupied: true,
            widthMm: 63.5,
            heightMm: 33.9,
            preview: grayPreview(packed),
          };
        });
    },
    async laPosteSlotDocument(data, format, slot) {
      ...
      if (!stamp)
        stamp = (JSON.parse(wasm.extractLaPostePdf(format, data, 203)) as Stamp[]).find(
          (item) => item.page === slot.sourcePage && item.slot === slot.slot,
        );
...
function parseBuildInfo(json: string): SdkBuildInfo {
  ...
  return {
    ...
    protocolSourceCommit: text(raw.protocolSourceCommit),
  };
}
...
function measurement(value: unknown): DocumentMeasurement {
  ...
  if (!Array.isArray(raw.elements) || typeof raw.layoutVersion !== 'string')
    throw new Error('The printer SDK returned an invalid document measurement.');
  const bounds = (input: unknown) => {
    ...
    if (![item.x, item.y, item.width, item.height].every(Number.isFinite))
      throw new Error('The printer SDK returned invalid physical bounds.');
    return {
      x: Number(item.x) / 1_000,
      y: Number(item.y) / 1_000,
      width: Number(item.width) / 1_000,
      height: Number(item.height) / 1_000,
    };
  };
  return {
    ...
    elements: raw.elements.map((input) => {
      ...
      if (
        ...
        typeof item.sourceElementId !== 'string' ||
        (item.zoneId !== undefined && typeof item.zoneId !== 'string')
      )
        throw new Error('The printer SDK returned an invalid measured element.');
      return {
...
function materializeOptions(options?: MaterializeOptions | ZoneBatchOptions) {
  ...
  return {
    locale: options?.locale ?? globalThis.navigator?.language ?? 'en',
    currentDate: options?.currentDate ?? currentDate,
    ...('zoneIds' in (options ?? {}) ? { zoneIds: (options as ZoneBatchOptions).zoneIds } : {}),
  };
}
function materializedDocument(value: unknown): LabelDocument {
  ...
  if (extensions && typeof extensions === 'object' && !Array.isArray(extensions)) {
    ...
    if (state && typeof state === 'object' && !Array.isArray(state)) {
      ...
      delete safeState.id;
      delete safeState.template;
      delete safeState.elements;
      (extensions as Record<string, unknown>)['makersbrain.editor:state'] = safeState;
    }
  }
  const document = fromSdkDocument(canonical);
  delete document.template;
  return document;
}

const sdkErrorCode = (error: unknown) =>
...
function grayPreview(page: NormalizedPage): RasterPreview {
  ...
  for (let index = 0; index < page.pixels.length; index++) {
    const gray = page.pixels[index];
...
async function grayPng(page: NormalizedPage) {
  const canvas = new OffscreenCanvas(page.rasterWidth, page.rasterHeight);
  const context = canvas.getContext('2d');
  if (!context) throw new Error('Canvas is unavailable.');
  context.putImageData(
    new ImageData(new Uint8ClampedArray(grayPreview(page).rgba), page.rasterWidth, page.rasterHeight),
    0,
    0,
  );
...
async function imageDocument(png: Uint8Array, title: string): Promise<LabelDocument> {
  ...
  document.media.width = 63.5;
  document.media.height = 33.9;
  document.media.printableBounds = { x: 0, y: 0, width: 63.5, height: 33.9 };
  const id = uuid();
  ...
  document.elements = [
    {
      ...
      resourceId: id,
      fit: 'stretch',
      transform: { x: 0, y: 0, width: 63.5, height: 33.9, rotation: 0 },
      zIndex: 0,
      visible: true,
      locked: false,
    },
