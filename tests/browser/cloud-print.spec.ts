...
const printer = (online: boolean) => ({ id: '4b066d0c-1f58-4149-a802-018714a606dd', agentId: 'agent-1', displayName: 'Packing desk', model: 'm110', enabled: true, online, lastSeenAt: 1 });
const job = (state: string, terminalOutcome: string | null = null) => ({
  id: 'b5e352b9-bab4-436f-bf81-da069cc164c0', printerId: printer(true).id, agentId: 'agent-1', state,
  terminalOutcome, progress: null, action: state, bytesSent: state === 'completed' ? 64 : 0, totalBytes: 64,
  lastCompletedAction: state === 'completed' ? 2 : -1, actionCount: 3, writeMayHaveOccurred: state === 'completed',
  cancellationRequestedAt: null, errorCode: null, createdAt: 1, deliveredAt: null, startedAt: null,
...
async function mockCloud(page: Page, online: boolean, submissions: { body: string; key: string | null }[]) {
  await page.route('https://print.example.test/**', async route => {
    ...
    if (request.url().endsWith('/printers')) return await route.fulfill({ status: 200, headers, body: JSON.stringify([printer(online)]) });
    ...
    if (request.method() === 'GET' && request.url().includes('/print-jobs/')) return await route.fulfill({ status: 200, headers, body: JSON.stringify(job('completed', 'completed')) });
