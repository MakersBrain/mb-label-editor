...
test('browser document library saves and reopens a label',async({page})=>{await page.goto('/');await page.getByText('Library',{exact:true}).click();await page.getByRole('button',{name:'Save to browser'}).click();await expect(page.getByText(/Saved Untitled label to this browser/)).toBeVisible();await page.getByRole('button',{name:'New label'}).click();await page.getByRole('button',{name:'Untitled label',exact:true}).click();await expect(page.getByText(/Opened Untitled label from this browser/)).toBeVisible()});
