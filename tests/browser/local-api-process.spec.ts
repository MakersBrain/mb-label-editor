...
const cliRoot =
  [
    resolve(import.meta.dirname, '../../../mb-printer-cli'),
    resolve(import.meta.dirname, '../../../../../mb-printer-cli'),
...
async function waitUntilReady(baseUrl: string, child: ChildProcess) {
  for (let attempt = 0; attempt < 80; attempt++) {
    ...
    try {
      await fetch(`${baseUrl}/capabilities`, { headers: { origin } });
      return;
    } catch {
      /* starting */
    }
...
async function stop(child: ChildProcess | undefined) {
  ...
  await Promise.race([
    once(child, 'exit'),
    new Promise((_, reject) => setTimeout(() => reject(new Error('mb-printer did not stop')), 5000)),
  ]);
}
...
test('real CLI process supports pairing, preflight, jobs, restart, and revocation', async () => {
  ...
  if (!existsSync(binary))
    execFileSync('cargo', ['build', '--manifest-path', join(cliRoot, 'Cargo.toml'), '--bin', 'mb-printer'], {
      stdio: 'pipe',
    });
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      max_document_bytes: 6_291_456,
      max_recent_jobs: 100,
      printer_defaults: {},
    }),
  );
  const pairOutput = execFileSync(binary, ['--config', config, 'service', 'pair', '--expires-seconds', '120'], {
    encoding: 'utf8',
  });
  const secret = pairOutput.trim().split(/\s+/).at(-1);
  expect(secret).toBeTruthy();
  let service: ChildProcess | undefined;
  const start = async () => {
    service = spawn(binary, ['--config', config, 'service', 'run', '--bind', '127.0.0.1', '--port', String(port)], {
      stdio: 'pipe',
    });
    await waitUntilReady(base, service);
  };
  try {
    await start();
    ...
    let connection: import('../../packages/label-editor/src/lib/print/local-api.js').LocalApiConnection | undefined;
    ...
    token = grant.token;
    connection = await route.configureConnection({
      id: 'acceptance-file',
      model: 'm200',
      transport: { kind: 'file', path: join(directory, 'printer.capture') },
    });
    ...
    const negotiated = await route.negotiateCapabilities();
    expect(negotiated.features).toContain('native-document-batch');
    connection = await route.configureConnection({
      id: 'continuous-file',
      model: 'ql-1110nwb',
      transport: { kind: 'file', path: join(directory, 'continuous.capture') },
    });
    ...
    const ql: PrinterDefinition = {
      ...
      continuousMedia: {
        ...
        requiredFeedBeforeMm: 0,
        requiredFeedAfterMm: 0,
      },
    };
    const batchProgress: {
      item: number;
      ...
      copy: number;
      copies: number;
      current: { action: number; bytesSent: number };
    }[] = [];
    expect(
      (
        await route.printBatch!({
          ...
          onProgress: (value) => batchProgress.push(value),
        })
      ).outcome,
    ).toBe('completed');
    ...
    expect(['cancelled-before-send', 'cancelled-partial', 'outcome-unknown', 'completed']).toContain(
    ...
    await start();
    expect((await route.validate(defaultDocument())).valid).toBe(true);
    const recovered = await fetch(`${base}/jobs/${submitted.id}`, {
      headers: { origin, authorization: `Bearer ${token}` },
    });
    expect(recovered.status).toBe(200);
    expect(((await recovered.json()) as { terminal: boolean }).terminal).toBe(true);
