...
const DB_NAME = 'makersbrain-label-editor';
const DB_VERSION = 1;
...
export class EditorDatabase {
  ...
  async put<T>(store: StoreName, key: IDBValidKey, value: T) {
    const db = await this.open();
    const transaction = db.transaction(store, 'readwrite');
    const done = transactionComplete(transaction);
    const request = requestResult(transaction.objectStore(store).put(structuredClone(value), key));
  ...
  async get<T>(store: StoreName, key: IDBValidKey): Promise<T | undefined> {
    ...
    const done = transactionComplete(transaction);
    const request = requestResult(transaction.objectStore(store).get(key));
    const [value] = await Promise.all([request, done]);
  ...
  async delete(store: StoreName, key: IDBValidKey) {
    ...
    const transaction = db.transaction(store, 'readwrite');
    const done = transactionComplete(transaction);
    const request = requestResult(transaction.objectStore(store).delete(key));
    await Promise.all([request, done]);
  }
  async entries<T>(store: StoreName): Promise<{ key: IDBValidKey; value: T }[]> {
    const db = await this.open();
    const transaction = db.transaction(store);
    const done = transactionComplete(transaction);
    const objectStore = transaction.objectStore(store);
    /* Queue both requests before yielding, while the transaction is active. */ const keysRequest = requestResult(
      objectStore.getAllKeys(),
    );
    const valuesRequest = requestResult(objectStore.getAll()) as Promise<T[]>;
    const [keys, values] = await Promise.all([keysRequest, valuesRequest, done]);
    return keys.map((key, index) => ({ key, value: values[index] }));
  }
  saveDocument(document: LabelDocument) {
    return this.put('documents', document.id, document);
  }
  async listDocuments() {
    return (await this.entries<LabelDocument>('documents'))
      .map((entry) => entry.value)
      .sort((a, b) => b.modifiedAt.localeCompare(a.modifiedAt));
  }
  async removeDocument(id: string) {
    await this.delete('documents', id);
    await this.delete('autosaves', id);
  }
  autosave(document: LabelDocument) {
    return this.put('autosaves', document.id, { document, savedAt: new Date().toISOString() });
  }
  ...
  saveTemplate(id: string, template: TemplateData) {
    return this.put('templates', id, template);
  }
  saveAsset(asset: Resource) {
    return this.put('assets', asset.sha256, asset);
  }
  saveJob(job: PersistedJob) {
    return this.put('jobs', job.id, job);
  }
  saveRecent(item: RecentItem) {
    return this.put('recent', `${item.kind}:${item.id}`, item);
  }
  async listRecent(limit = 20) {
    return (await this.entries<RecentItem>('recent'))
      .map((entry) => entry.value)
      .sort((a, b) => b.openedAt.localeCompare(a.openedAt))
      .slice(0, limit);
  }
  savePreferences(preferences: EditorPreferences) {
    return this.put('preferences', 'editor', preferences);
  }
  getPreferences() {
    return this.get<EditorPreferences>('preferences', 'editor');
  }
}
