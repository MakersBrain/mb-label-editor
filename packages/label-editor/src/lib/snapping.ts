...
export function snapTargets(elements: LabelElement[], selected: Set<string>, media: Bounds, options: Pick<SnapOptions, 'guides' | 'zones'>, document?: LabelDocument): SnapTargets {
  ...
  const edges = (bounds: Bounds, axis: 'x' | 'y') => (axis === 'x' ? [bounds.x, bounds.x + bounds.width / 2, bounds.x + bounds.width] : [bounds.y, bounds.y + bounds.height / 2, bounds.y + bounds.height]);
  const stationaryBounds = stationary.map(rootBounds);
  return {
    moving: moving.length ? { x: Math.min(...movingBounds.map((item) => item.x)), y: Math.min(...movingBounds.map((item) => item.y)), width: Math.max(...movingBounds.map((item) => item.x + item.width)) - Math.min(...movingBounds.map((item) => item.x)), height: Math.max(...movingBounds.map((item) => item.y + item.height)) - Math.min(...movingBounds.map((item) => item.y)) } : undefined,
    media: { x: edges(media, 'x'), y: edges(media, 'y') },
    elements: { x: stationaryBounds.flatMap((bounds) => edges(bounds, 'x')), y: stationaryBounds.flatMap((bounds) => edges(bounds, 'y')) },
    zones: { x: zones.flatMap((zone) => edges(zone, 'x')), y: zones.flatMap((zone) => edges(zone, 'y')) },
    guides: { x: options.guides.filter((guide) => guide.axis === 'x').map((guide) => guide.value), y: options.guides.filter((guide) => guide.axis === 'y').map((guide) => guide.value) },
  };
}

/** Snaps a proposed delta against precomputed targets; cheap enough to run on every pointer move. */
export function snapWithTargets(targets: SnapTargets, delta: Point, options: Pick<SnapOptions, 'grid' | 'gridEnabled' | 'threshold' | 'mode'>): SnapResult {
  const mode = options.mode ?? 'all';
  if (mode === 'none' || !targets.moving) return { delta, guides: [] };
  const { moving } = targets;
  const x0 = moving.x; const x1 = moving.x + moving.width; const y0 = moving.y; const y1 = moving.y + moving.height;
  const allTargets = mode === 'all';
  const elementTargets = mode === 'elements' || allTargets;
  const xTargets = [...(allTargets ? targets.media.x : []), ...(elementTargets ? targets.elements.x : []), ...(allTargets ? targets.zones.x : []), ...(allTargets ? targets.guides.x : [])];
  const yTargets = [...(allTargets ? targets.media.y : []), ...(elementTargets ? targets.elements.y : []), ...(allTargets ? targets.zones.y : []), ...(allTargets ? targets.guides.y : [])];
  const snapAxis = (edges: number[], candidates: number[], proposed: number) => { let correction = 0; let guide: number | undefined; let distance = options.threshold;
    for (const edge of edges) for (const target of candidates) { const candidate = target - (edge + proposed); if (Math.abs(candidate) <= distance) { distance = Math.abs(candidate); correction = candidate; guide = target; } }
...
export const guidesEqual = (a: SnapResult['guides'], b: SnapResult['guides']): boolean => a.length === b.length && a.every((guide, index) => guide.axis === b[index].axis && guide.value === b[index].value);
