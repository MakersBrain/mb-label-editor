...
function fitGroupsToChildren(document: LabelDocument): void {
  ...
  const depth = (element: LabelElement): number => {
    let level = 0;
    let current = element;
  ...
  const groups = document.elements
    .filter((element) => element.type === 'group')
    .map((group) => ({ group, depth: depth(group) }))
    .sort((a, b) => b.depth - a.depth);
  for (const { group } of groups) {
    const children = group.childIds.flatMap((id) => {
      const child = byId.get(id);
      return child ? [child] : [];
    });
    if (!children.length) {
      group.transform = { ...group.transform, width: 0, height: 0 };
      continue;
    }
...
const elementById = (doc: LabelDocument, id: Id): LabelElement => {
  ...
  if (!item) throw new Error(`Unknown element ${id}`);
  return item;
};
...
export function assertGroupInvariants(document: LabelDocument): void {
  ...
  const groups = new Map(
    document.elements.filter((element) => element.type === 'group').map((group) => [group.id, group]),
  );
  for (const element of document.elements) {
    ...
    if (!parent.childIds.includes(element.id))
      throw new Error(`Group ${parent.id} does not contain child ${element.id}`);
  }
...
export const addElement = (element: LabelElement): Command => ({
  ...
  apply: (doc) =>
    changed(doc, (copy) => {
      copy.elements.push(structuredClone(element));
    }),
});
...
export const patchElement = (id: Id, patch: Partial<LabelElement>): Command => ({
  label: 'Edit element',
  coalesceKey: `edit:${id}:${Object.keys(patch).sort().join(',')}`,
  apply: (doc) =>
    changed(doc, (copy) => {
      if (['id', 'type', 'groupId', 'childIds'].some((key) => Object.hasOwn(patch, key)))
        throw new Error('Element identity and group links require a dedicated command');
...
export const transformElements = (
  ...
  const selected = new Set(ids);
  const coalesceKey = `${operation}:${[...selected].sort().join(',')}`;
  return {
    label: 'Transform elements',
    coalesceKey,
    apply: (doc) =>
      changed(doc, (copy) =>
        copy.elements.forEach((element) => {
          if (selected.has(element.id) && !isEffectivelyLocked(copy, element))
...
export const moveElements = (ids: Iterable<Id>, delta: Point): Command => {
  const selected = [...new Set(ids)];
  const coalesceKey = `move:${[...selected].sort().join(',')}`;
  return {
    label: 'Move elements',
    coalesceKey,
    apply: (doc) =>
...
export const resizeElement = (id: Id, size: { width: number; height: number }, origin?: Point): Command =>
  transformElements(
    [id],
    (current) => ({
      ...current,
      ...(origin ?? {}),
      width: Math.max(0.1, size.width),
      height: Math.max(0.1, size.height),
    }),
    'resize',
  );
export const resizeElements = (ids: Iterable<Id>, bounds: Bounds): Command => {
  const selected = [...new Set(ids)];
  return {
    label: 'Resize elements',
    coalesceKey: `resize:${[...selected].sort().join(',')}`,
    apply: (doc) =>
      changed(doc, (copy) => {
        ...
        const source = elementBounds(
          items.map((item) => ({ ...item, transform: { ...item.transform, ...elementRootBounds(copy, item) } })),
        );
        const scaleX = bounds.width / Math.max(0.1, source.width);
        const scaleY = bounds.height / Math.max(0.1, source.height);
        const resizing = descendantIds(
          copy,
...
export const rotateElements = (ids: Iterable<Id>, degrees: number): Command =>
  transformElements(ids, (current) => ({ ...current, rotation: ((degrees % 360) + 360) % 360 }), 'rotate');
  ...
  return {
    ...
    apply: (doc) =>
      changed(doc, (copy) => {
        ...
        const source = copy.elements.filter((item) => selected.has(item.id));
        const replacements = new Map(source.map((item) => [item.id, uuid()]));
        const firstZIndex = copy.elements.length;
        for (const [index, item] of source.entries()) {
          const duplicate = structuredClone(item);
          duplicate.id = replacements.get(item.id)!;
          duplicate.name = `${item.name} copy`;
          duplicate.transform.x += offset.x;
          duplicate.transform.y += offset.y;
          duplicate.zIndex = firstZIndex + index;
          if (duplicate.groupId && replacements.has(duplicate.groupId))
            duplicate.groupId = replacements.get(duplicate.groupId);
          else delete duplicate.groupId;
...
export const alignElements = (ids: Iterable<Id>, alignment: Alignment): Command => {
  ...
  return {
    ...
    apply: (doc) =>
      changed(doc, (copy) => {
        const items = copy.elements.filter((item) => selected.has(item.id) && !isEffectivelyLocked(copy, item));
        ...
        items.forEach((item) => {
          ...
          if (alignment === 'left') t.x = left;
          if (alignment === 'right') t.x = right - t.width;
          if (alignment === 'center-x') t.x = (left + right - t.width) / 2;
          if (alignment === 'top') t.y = top;
          if (alignment === 'bottom') t.y = bottom - t.height;
          if (alignment === 'center-y') t.y = (top + bottom - t.height) / 2;
        });
      }),
  };
};
...
export const alignElementsToBounds = (ids: Iterable<Id>, alignment: Alignment, bounds: Bounds): Command => {
  const selected = [...new Set(ids)];
  return {
    label: `Align ${alignment} to boundary`,
    apply: (doc) =>
      changed(doc, (copy) => {
...
export const distributeElements = (ids: Iterable<Id>, axis: 'horizontal' | 'vertical'): Command => {
  ...
  return {
    ...
    apply: (doc) =>
      changed(doc, (copy) => {
        const items = copy.elements
          .filter((item) => selected.has(item.id) && !isEffectivelyLocked(copy, item))
          .sort((a, b) => (axis === 'horizontal' ? a.transform.x - b.transform.x : a.transform.y - b.transform.y));
        ...
        const space =
          (position(items.at(-1)!) +
            extent(items.at(-1)!) -
            position(items[0]) -
            items.reduce((sum, item) => sum + extent(item), 0)) /
          (items.length - 1);
        let cursor = position(items[0]) + extent(items[0]) + space;
        for (const item of items.slice(1, -1)) {
          if (axis === 'horizontal') item.transform.x = cursor;
          else item.transform.y = cursor;
          cursor += extent(item) + space;
        }
      }),
  };
};
export const reorderElement = (id: Id, target: 'front' | 'back' | 'forward' | 'backward'): Command => ({
  ...
  apply: (doc) =>
    changed(doc, (copy) => {
      const item = elementById(copy, id);
      const ordered = [...copy.elements].sort((a, b) => a.zIndex - b.zIndex);
  ...
  return {
    ...
    apply: (doc) =>
      changed(doc, (copy) => {
        ...
        const children = selected.map((id) => elementById(copy, id));
        if (children.some((item) => item.groupId))
          throw new Error('Ungroup nested elements before grouping them again');
        ...
        copy.elements.push({
          ...
          name: 'Group',
          childIds: selected,
          transform: { x, y, width: right - x, height: bottom - y, rotation: 0 },
          zIndex: Math.max(...children.map((item) => item.zIndex)) + 1,
          visible: true,
          locked: false,
        });
        assertGroupInvariants(copy);
...
const isInside = (document: LabelDocument, id: Id, ancestorId: Id): boolean => {
  let current = elementById(document, id);
  while (current.groupId) {
    if (current.groupId === ancestorId) return true;
    current = elementById(document, current.groupId);
  }
  return false;
};
...
export const moveToGroup = (ids: Iterable<Id>, groupId: Id | undefined): Command => {
  ...
  return {
    label: groupId ? 'Move into group' : 'Move out of group',
    apply: (doc) =>
      changed(doc, (copy) => {
        ...
        for (const id of moved) {
          const item = elementById(copy, id);
          if (target && (id === target.id || isInside(copy, target.id, id)))
            throw new Error('A group cannot be moved into itself');
          if (item.groupId === groupId) continue;
          if (item.groupId) {
            const parent = elementById(copy, item.groupId);
            if (parent.type === 'group') parent.childIds = parent.childIds.filter((child) => child !== id);
          }
...
export const ungroup = (groupId: Id): Command => ({
  label: 'Ungroup elements',
  apply: (doc) =>
    changed(doc, (copy) => {
      const group = elementById(copy, groupId);
      ...
      copy.elements.forEach((item) => {
        if (item.groupId === groupId) {
          if (parent) item.groupId = parent.id;
          else delete item.groupId;
        }
      });
...
export const updateDocument = (
  patch: Partial<Omit<LabelDocument, 'version' | 'elements' | 'resources' | 'fonts'>>,
  coalesceKey?: string,
): Command => ({
  label: 'Edit document',
  coalesceKey,
  apply: (doc) => changed(doc, (copy) => Object.assign(copy, structuredClone(patch))),
});
...
export const addResource = (resource: LabelDocument['resources'][number]): Command => ({
  label: 'Import asset',
  apply: (doc) =>
    changed(doc, (copy) => {
      if (!copy.resources.some((item) => item.id === resource.id))
        copy.resources = [...copy.resources, structuredClone(resource)];
    }),
});
export const addFont = (font: LabelDocument['fonts'][number]): Command => ({
  label: 'Import font',
  apply: (doc) =>
    changed(doc, (copy) => {
      if (!copy.fonts.some((item) => item.sha256 === font.sha256)) copy.fonts = [...copy.fonts, structuredClone(font)];
    }),
});
