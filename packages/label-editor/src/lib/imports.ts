...
import { uuid, type FontResource, type Resource } from './model.js';
...
async function toPng(data: Uint8Array, mimeType: string, halftone: boolean): Promise<Uint8Array> {
  ...
  try {
    bitmap = await createImageBitmap(new Blob([data.slice().buffer as ArrayBuffer], { type: mimeType }));
  } catch {
  ...
  if (halftone) {
    ...
    context.putImageData(ditherImage(image), 0, 0);
  }
...
export function ditherImage(image: ImageData): ImageData {
  ...
  for (let index = 0; index < width * height; index += 1) {
    ...
    const luma = 0.299 * data[index * 4] + 0.587 * data[index * 4 + 1] + 0.114 * data[index * 4 + 2];
    gray[index] = luma * alpha + 255 * (1 - alpha);
  }
  ...
  for (let index = 0; index < width * height; index += 1) {
    data[index * 4] = data[index * 4 + 1] = data[index * 4 + 2] = gray[index];
    data[index * 4 + 3] = 255;
  }
  return image;
}
export async function importAsset(
  ...
  sdk?: PrinterSdk,
  dpi = 300,
  options?: { halftone?: boolean },
): Promise<{ resource: Resource; widthMm?: number; heightMm?: number }> {
  let data: Uint8Array = bytes(await file.arrayBuffer());
  let mimeType = file.type || mimeFromName(file.name);
  let name = file.name;
  let widthMm: number | undefined;
  let heightMm: number | undefined;
  if (mimeType === 'application/pdf') {
    if (!sdk) throw new Error('Offline PDF import requires the installed printer SDK.');
    const page = await sdk.importFirstPdfPage(data, dpi);
    data = page.data;
  ...
  if (mimeType === 'image/svg+xml') {
    ...
    if (/(?:href|src)\s*=\s*["'](?:https?:|\/\/)/i.test(source) || /<script\b/i.test(source))
      throw new Error('SVG must not contain scripts or external resources.');
  }
  ...
  if (halftone || !nativeImageTypes.includes(mimeType)) {
    data = await toPng(data, mimeType, halftone);
    mimeType = 'image/png';
  }
...
export async function importFont(
  file: File,
  details?: { family?: string; weight?: number; style?: 'normal' | 'italic' },
): Promise<FontResource> {
  const data = bytes(await file.arrayBuffer());
  const mimeType = fontMimeType(data, file.name, file.type);
  if (!mimeType) throw new Error(`Unsupported font type: ${file.type || mimeFromName(file.name)}`);
  return {
    id: uuid(),
    name: file.name,
    mimeType,
    sha256: await sha256(new Uint8Array(data).buffer),
    data: base64(data),
    family: details?.family ?? file.name.replace(/\.[^.]+$/, ''),
    weight: details?.weight ?? 400,
    style: details?.style ?? 'normal',
  };
}
