...
export const GENERIC_CONTINUOUS_LIMITS: ContinuousMediaLimits = Object.freeze({ minimumLengthMm: 0.1, maximumLengthMm: 1_000, source: 'generic-export' });
...
export type ContinuousWarningCode = 'continuous.content_before_leading_margin' | 'continuous.content_after_cut_line' | 'continuous.empty_content' | 'continuous.not_printer_qualified';
export interface ContinuousMediaWarning { code: ContinuousWarningCode; severity: 'warning' | 'information'; message: string }
...
export class ContinuousMediaError extends Error {
  constructor(readonly code: ContinuousErrorCode, message: string) { super(message); this.name = 'ContinuousMediaError'; }
}
...
export function assertDocumentReadyForOutput(document: LabelDocument): asserts document is ResolvedLabelDocument {
  if (document.media.shape === 'continuous' && !isResolvedLabelDocument(document)) {
    throw new ContinuousMediaError('continuous.unresolved_document', 'Continuous labels must be prepared before rendering, export, or printing.');
  }
}

export function documentLayoutFingerprint(document: LabelDocument): string {
  return hashStable({ id: document.id, media: document.media, elements: document.elements, resources: document.resources.map(({ id, sha256 }) => ({ id, sha256 })), fonts: document.fonts.map(({ id, sha256 }) => ({ id, sha256 })) });
}
...
export function contentBounds(_document:LabelDocument,measurement:DocumentMeasurement):Bounds|undefined{return measuredContentBounds(measurement)}

/** Validates authored continuous settings independently of layout measurement. */
export function validateContinuousMedia(document:LabelDocument,limits:ContinuousMediaLimits=GENERIC_CONTINUOUS_LIMITS):ContinuousMediaWarning[]{
  validateLimits(limits);if(document.media.shape!=='continuous')return[];
  if(!finitePositive(document.media.width))throw new ContinuousMediaError('continuous.invalid_width','Continuous roll width must be greater than zero.');
  const settings=continuousSettings(document);const minimum=Math.max(limits.minimumLengthMm,settings.preferredMinimumLengthMm??limits.minimumLengthMm);const maximum=Math.min(limits.maximumLengthMm,settings.preferredMaximumLengthMm??limits.maximumLengthMm);
  if(!finitePositive(minimum)||!finitePositive(maximum)||minimum>maximum)throw new ContinuousMediaError('continuous.invalid_length_range','Continuous media minimum and maximum lengths are invalid.');
  nonNegative(settings.leadingMarginMm,'Leading margin');nonNegative(settings.trailingMarginMm,'Trailing margin');
  if(settings.lengthMode==='fixed'&&(!finitePositive(settings.fixedLengthMm)||settings.fixedLengthMm<minimum||settings.fixedLengthMm>maximum))throw new ContinuousMediaError('continuous.invalid_fixed_length',`Fixed cut length must be between ${minimum} and ${maximum} mm.`);
  const printable=document.media.printableBounds;const bottomInset=document.media.height-printable.y-printable.height;if(![printable.y,bottomInset].every(Number.isFinite)||printable.y<0||bottomInset<0)throw new ContinuousMediaError('continuous.invalid_printable_bounds','Continuous media printable bounds contain invalid top or bottom insets.');
  return limits.source==='generic-export'?[{code:'continuous.not_printer_qualified',severity:'information',message:'Length was checked against generic export limits and must be validated for the destination printer.'}]:[];
}
...
export function resolveContinuousDocument(
  ...
  const effectiveMinimum = Math.max(limits.minimumLengthMm, settings.preferredMinimumLengthMm ?? limits.minimumLengthMm);
  const effectiveMaximum = Math.min(limits.maximumLengthMm, settings.preferredMaximumLengthMm ?? limits.maximumLengthMm);
  if (!finitePositive(effectiveMinimum) || !finitePositive(effectiveMaximum) || effectiveMinimum > effectiveMaximum) {
    throw new ContinuousMediaError('continuous.invalid_length_range', 'Continuous media minimum and maximum lengths are invalid.');
  }
  ...
  if (document.media.shape === 'continuous') {
    ...
    if (settings.lengthMode === 'fixed') {
      if (!finitePositive(settings.fixedLengthMm)) throw new ContinuousMediaError('continuous.invalid_fixed_length', 'Fixed cut length must be greater than zero.');
      naturalLength = roundMillimetres(settings.fixedLengthMm);
      if (naturalLength < effectiveMinimum || naturalLength > effectiveMaximum) {
        throw new ContinuousMediaError('continuous.invalid_fixed_length', `Fixed cut length must be between ${effectiveMinimum} and ${effectiveMaximum} mm.`);
      }
      resolvedLength = naturalLength;
      if (measurement) {
        bounds = measuredContentBounds(measurement);
        if (bounds && bounds.y + bounds.height > resolvedLength) warnings.push({ code: 'continuous.content_after_cut_line', severity: 'warning', message: 'Visible content extends past the fixed cut line.' });
      }
    } else {
      if (!measurement) throw new ContinuousMediaError('continuous.measurement_unavailable', 'Fit content requires authoritative SDK layout measurement.');
      bounds = measuredContentBounds(measurement);
      if (!bounds) {
        naturalLength = effectiveMinimum;
        warnings.push({ code: 'continuous.empty_content', severity: 'information', message: 'The empty continuous label uses its minimum length.' });
      } else {
        naturalLength = roundMillimetres(bounds.y + bounds.height + nonNegative(settings.trailingMarginMm, 'Trailing margin'));
        if (bounds.y < nonNegative(settings.leadingMarginMm, 'Leading margin')) {
          warnings.push({ code: 'continuous.content_before_leading_margin', severity: 'warning', message: 'Visible content crosses the leading safe margin.' });
        }
      }
      if (naturalLength > effectiveMaximum) {
        throw new ContinuousMediaError('continuous.content_exceeds_maximum', `Content requires ${naturalLength.toFixed(3)} mm, exceeding the ${effectiveMaximum.toFixed(3)} mm maximum.`);
      }
  ...
  return { document: copy, contentBounds: bounds, naturalLengthMm: naturalLength, resolvedLengthMm: resolvedLength, warnings };
}
...
export function resolveContinuousBatch(
  ...
  if (documents.length !== measurements.length) throw new Error('Each continuous batch document requires a matching measurement entry.');
  const results = documents.map((document, index) => resolveContinuousDocument(document, measurements[index], limits, index));
...
function measuredContentBounds(measurement: DocumentMeasurement): Bounds | undefined {
  ...
  if (![bounds.x, bounds.y, bounds.width, bounds.height].every(Number.isFinite) || bounds.width < 0 || bounds.height < 0) {
    throw new ContinuousMediaError('continuous.measurement_unavailable', 'The SDK returned invalid measured content bounds.');
  }
...
function updateMediaLength(document: LabelDocument, height: number): void {
  if (!finitePositive(height)) throw new ContinuousMediaError('continuous.invalid_fixed_length', 'Resolved media length must be greater than zero.');
  ...
  if (![topInset, bottomInset, nextPrintableHeight].every(Number.isFinite) || topInset < 0 || bottomInset < 0 || nextPrintableHeight <= 0) {
    throw new ContinuousMediaError('continuous.invalid_printable_bounds', 'Continuous media printable bounds cannot preserve their top and bottom insets at the resolved length.');
  }
  document.media.height = height;
  document.media.printableBounds.height = nextPrintableHeight;
}

function roundMillimetres(value:number):number{return Math.round((value+Number.EPSILON)*1_000)/1_000}

function validateLimits(limits: ContinuousMediaLimits): void {
  if (!finitePositive(limits.minimumLengthMm) || !finitePositive(limits.maximumLengthMm) || limits.minimumLengthMm > limits.maximumLengthMm) {
    throw new ContinuousMediaError('continuous.invalid_length_range', 'Continuous media capability limits are invalid.');
  }
}
...
function attachResolutionStamp(document: ResolvedLabelDocument, stamp: ResolutionStamp): void {
  Object.defineProperty(document, RESOLVED_LABEL_DOCUMENT, { value: Object.freeze(stamp), enumerable: false, configurable: false, writable: false });
}
function nonNegative(value: number, label: string): number {
  if (!Number.isFinite(value) || value < 0) throw new ContinuousMediaError('continuous.invalid_length_range', `${label} must be zero or greater.`);
  return value;
}
function fingerprint(document: LabelDocument, measurement: DocumentMeasurement | undefined, settings: ContinuousMediaSettingsV1, limits: ContinuousMediaLimits): string {
...
export function normalizeContinuousMeasurementError(error: unknown): ContinuousMediaError {
  ...
  if (/invalid zone|zone cycle|group or zone cycle|clone.zone|clone zone/i.test(message)) {
    return new ContinuousMediaError('continuous.invalid_zone', `Continuous layout contains an invalid zone or clone reference: ${message}`);
  }
