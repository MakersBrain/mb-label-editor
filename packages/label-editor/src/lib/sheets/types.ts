...
export function isSheetLayoutPreset(value: unknown): value is SheetLayoutPreset {
  ...
  if (typeof preset.id !== 'string' || !preset.id.trim() || typeof preset.name !== 'string' || !preset.name.trim()
    || !['a4', 'letter', 'custom'].includes(String(preset.paper))
    || !['portrait', 'landscape'].includes(String(preset.orientation))) return false;
  if (Number(!!preset.grid) + Number(!!preset.slots) !== 1) return false;
  if (preset.grid) {
    const grid = preset.grid;
    return Number.isSafeInteger(grid.rows) && grid.rows > 0
  ...
  return Array.isArray(preset.slots) && preset.slots.length > 0 && preset.slots.every((slot) =>
...
export function isSheetPlan(value: unknown): value is SheetPlan {
  ...
  if (!(Number.isSafeInteger(plan.pageCount) && plan.pageCount! > 0
    && !!layout && typeof layout.id === 'string'
    && Number.isSafeInteger(layout.paperWidthUm) && layout.paperWidthUm > 0
    && Number.isSafeInteger(layout.paperHeightUm) && layout.paperHeightUm > 0
    && Array.isArray(layout.slots) && layout.slots.length > 0 && layout.slots.every((slot) => isSheetSlot(slot)
      && slot.xUm <= layout.paperWidthUm - slot.widthUm
      && slot.yUm <= layout.paperHeightUm - slot.heightUm)
    && Array.isArray(plan.placements) && plan.placements.length > 0 && plan.placements.every((placement) =>
      Number.isSafeInteger(placement.item) && placement.item >= 0
      && Number.isSafeInteger(placement.page) && placement.page >= 0 && placement.page < plan.pageCount!
      && Number.isSafeInteger(placement.slot) && placement.slot >= 0 && placement.slot < layout.slots.length))) return false;
...
export function isSheetPlanForRequest(value: unknown, input: SheetPlanInput, definition: SheetDefinition, options: SheetOptions): value is SheetPlan {
  ...
  if (options.firstSlot >= expectedSlots.length || value.placements.length !== input.itemCount
    ...
    || value.layout.slots.some((slot, index) => !sameSlot(slot, expectedSlots[index]))) return false;
...
function isSheetSlot(value: unknown): value is SheetSlot {
  ...
  return [slot.xUm, slot.yUm, slot.widthUm, slot.heightUm].every(Number.isSafeInteger)
    && slot.xUm! >= 0 && slot.yUm! >= 0 && slot.widthUm! > 0 && slot.heightUm! > 0;
}
...
function expandGridSlots(grid: SheetGridDefinition): SheetSlot[] {
  ...
  const append = (row: number, column: number) => slots.push({
...
const sameSlot = (left: SheetSlot, right: SheetSlot) => left.xUm === right.xUm && left.yUm === right.yUm && left.widthUm === right.widthUm && left.heightUm === right.heightUm;
