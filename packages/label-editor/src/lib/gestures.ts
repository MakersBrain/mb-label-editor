...
export class GestureTracker {
  ...
  start(id: number, point: Point) {
    this.#points.set(id, point);
    this.#previous = this.measure();
  }
  move(id: number, point: Point): GestureUpdate | undefined {
    ...
    return {
      panDelta: { x: after.center.x - before.center.x, y: after.center.y - before.center.y },
      zoomFactor: before.distance && after.distance ? after.distance / before.distance : 1,
      center: after.center,
    };
  }
  end(id: number) {
  ...
  private measure() {
    ...
    if (values.length === 1) return { center: values[0], distance: 0 };
    const [a, b] = values;
