...
<script lang="ts">import{onMount}from'svelte';import type{EditorStore}from'../store.js';import{patchElement,updateDocument}from'../commands.js';import{parseCsv}from'../template/csv.js';import{evaluateTemplate}from'../template/evaluate.js';import{EditorDatabase}from'../persistence/database.js';import type{TemplateData}from'../model.js';import DataSheet from'./DataSheet.svelte';export let editor:EditorStore;export let onSyntaxHelp:()=>void=()=>{};const database=new EditorDatabase();let error='';let templates:{key:IDBValidKey,value:TemplateData}[]=[];onMount(refresh);async function refresh(){templates=await database.entries<TemplateData>('templates')}async function load(event:Event){try{const file=(event.currentTarget as HTMLInputElement).files?.[0];if(!file)return;const parsed=parseCsv(await file.text());const template={...parsed,currentRecord:0};editor.execute(updateDocument({template}));await database.saveTemplate($editor.document.id,template);await database.saveRecent({id:$editor.document.id,kind:'template',openedAt:new Date().toISOString()});await refresh();error=''}catch(reason){error=reason instanceof Error?reason.message:String(reason)}}
...
<section class="data">
  <h2>Data</h2>
  <p class="help">Each record fills the label once. Values such as <code>{'{{'}price | number:2{'}}'}</code> are evaluated per record. <button type="button" class="link" on:click={onSyntaxHelp}>Template syntax reference</button></p>
  <div class="sources">
    <label class="upload">Import CSV<input type="file" accept=".csv,text/csv" on:change={load}></label>
    {#if templates.length}<label class="saved">Saved data<select on:change={e=>{const item=templates[+e.currentTarget.value];if(item)editor.execute(updateDocument({template:structuredClone(item.value)}))}}><option value="">Choose saved data</option>{#each templates as item,index}<option value={index}>{String(item.key)} · {item.value.records.length} records</option>{/each}</select></label>{/if}
  ...
  {#if $editor.document.template}
    ...
    <details open class="section"><summary>Preview record</summary>
      <label>Record <input type="range" min="0" max={Math.max(0,template.records.length-1)} value={template.currentRecord} on:input={(e)=>record(+e.currentTarget.value)}> <span class="muted">{template.currentRecord+1} of {template.records.length}</span></label>
