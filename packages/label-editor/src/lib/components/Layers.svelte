...
<script lang="ts">
  ...
  import { createGroup, moveToGroup, reorderElement, setLocked, setVisibility } from '../commands.js';
    ...
    {#each list as row (row.element.id)}
      ...
      <li
        ...
        ondragover={(event) => {
          ...
          ((e) => dragOver(e, element.id))(event);
        }}
        ondrop={(event) => {
          event.stopPropagation();
          ((e) => drop(e, element))(event);
        }}
      >
        ...
        <button class="name" onclick={(e) => editor.select([element.id], e.shiftKey)}
