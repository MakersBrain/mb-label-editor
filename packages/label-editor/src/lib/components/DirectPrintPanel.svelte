...
<script lang="ts">
  ...
  interface Props {
    ...
    localRoute?: LocalApiPrintRoute;
    localConnection?: LocalApiConnection;
    onLocalConnection?: (connection: LocalApiConnection | undefined) => void;
    onConfigureLocal?: () => void;
    onSelectLocal?: () => void;
    onRoute?: (route: PrintRoute) => void;
    onMedia?: (media: { width: number; height: number; shape: 'rectangle' | 'round' | 'continuous' }) => void;
    initialContinuous?: ContinuousPrintOptions;
    onContinuous?: (options: ContinuousPrintOptions) => void;
  }
  let {
    sdk,
    materializer,
    ...
    onRoute = () => {},
    onMedia = () => {},
    initialContinuous = { cutMode: 'after-each', extraFeedBeforeMm: 0, extraFeedAfterMm: 0, chainCopies: false },
    onContinuous = () => {},
  }: Props = $props();
  ...
  $effect(() => {
    const id = printer?.id ?? '';
    const initial = initialContinuous;
    untrack(() => syncContinuousPreference(id, initial));
  });
  ...
  function syncContinuousPreference(printerId: string, value: ContinuousPrintOptions) {
    const key = `${printerId}:${value.cutMode}:${value.extraFeedBeforeMm}:${value.extraFeedAfterMm}:${value.chainCopies}`;
    if (key === continuousPreferenceKey) return;
    continuousPreferenceKey = key;
    cutMode = value.cutMode;
    extraFeedBeforeMm = value.extraFeedBeforeMm;
    extraFeedAfterMm = value.extraFeedAfterMm;
    chainCopies = value.chainCopies;
  }

  const preferredRoute = (
    definition: PrinterDefinition | undefined,
    saved: LocalApiConnection | undefined,
  ): RouteKind =>
    saved && (!definition || saved.model === definition.id)
      ? 'local'
      : definition?.protocols[0] === 'brother'
        ? 'usb'
        : 'bluetooth';
  $effect(() => {
    if (!routePinned && connection === 'disconnected') {
      const next = preferredRoute(printer, localConnection);
      untrack(() => {
        route = next;
      });
    }
  ...
  $effect(() => {
    if (connectedPrinterId && printer?.id !== connectedPrinterId)
      untrack(() => disconnect('Printer changed. Connect the new printer.')).catch(() => {});
  });
  const displayConnection = $derived(route === 'local' ? (localConnection ? 'connected' : 'disconnected') : connection);
  const reportedMedia = $derived(
    route === 'local' ? localMediaDescription(localConnection, document) : mediaDescription(printerStatus, document),
  );
  ...
  const availableCutModes = $derived(
    rollCapabilities?.cutModes.filter((mode) => mode !== 'after-job' || activeRoute?.supportsNativeBatch !== false) ??
      [],
  );
  $effect(() => {
    if (rollCapabilities && !availableCutModes.includes(cutMode)) {
      const next = availableCutModes[0] ?? 'none';
      untrack(() => {
        cutMode = next;
      });
    }
  ...
  function makeRoute() {
    ...
    if (route === 'bluetooth')
      return new DirectPrintRoute(
        sdk,
        async () => new WebBluetoothTransport({ service, writeCharacteristic: write, notifyCharacteristic: notify }),
        'bluetooth',
    ...
    if (route === 'spp')
      return new DirectPrintRoute(
        sdk,
        async () => new WebSerialTransport({ physicalWriteLimit: 512, unfiltered: true }),
        'serial',
    ...
    return new DirectPrintRoute(
      sdk,
      async () => new WebUsbTransport({ filters: usbFilters(), interfaceNumber: 0 }),
      'usb',
  ...
  async function connect() {
    if (!printer) {
      message = 'Select a printer model first.';
      return;
    }
    if (route === 'local') {
      if (!localConnection || !localRoute) {
        message = 'Configure an IPP/IPPS printer in the local service first.';
        onConfigureLocal();
        return;
      }
      onSelectLocal();
  ...
  async function print() {
    if (!printer) {
      message = 'Select a printer model before printing.';
      return;
    }
    const selected = route === 'local' ? localRoute : active;
    if (
      !selected ||
      (route === 'local' ? !localConnection || localConnection.model !== printer.id : !active?.connected)
    ) {
      message = 'Connect the selected printer before printing.';
      return;
    }
    ...
    try {
      ...
      const result = await selected.print({
        document: prepared.document,
        printer,
        copies: 1,
        ...(rollCapabilities ? { continuous: { cutMode, extraFeedBeforeMm, extraFeedAfterMm, chainCopies } } : {}),
        signal: cancellation.signal,
        onProgress: (progress) => (message = `${progress.phase}: ${progress.bytesSent}/${progress.totalBytes} bytes`),
      });
      message =
        result.outcome === 'completed'
          ? `Printed ${result.bytesSent} bytes.`
          : `${result.outcome}: ${result.error ?? 'Check the physical printer before retrying.'}`;
    ...
    if (route === 'local') {
      ...
      try {
        ...
        message = current.connected
          ? `Connected · ${describeLocal(updated)}`
          : 'Saved printer is currently unavailable.';
        if (apply) applyLocalMedia(updated);
      } catch (error) {
        message = `Local printer status is unavailable: ${error instanceof Error ? error.message : String(error)}`;
      } finally {
        querying = false;
      }
      return;
    }
    if (!printer || !active?.connected) {
      message = 'Connect the selected printer first.';
      return;
    }
    querying = true;
    try {
  ...
  function localMediaDescription(value: LocalApiConnection | undefined, doc: LabelDocument) {
    ...
    return {
      name: typeof media.keyword === 'string' ? media.keyword : undefined,
      width,
      length,
      kind: media.lengthMm === 0 ? 'continuous' : doc.media.shape,
    };
  }
  ...
  function applyLocalMedia(value: LocalApiConnection) {
    ...
    onMedia({
      width: media.width,
      height: media.length,
      shape: media.kind === 'continuous' ? 'continuous' : 'rectangle',
    });
  }
</script>
...
<section class="printer-card" data-state={displayConnection}>
  ...
  <label
    >Connection<select bind:value={route} onchange={changeRoute} disabled={connection === 'connecting'}
      >{#if localRoute}<option value="local"
          >{localConnection
            ? `${localConnection.transport.kind.toUpperCase()} · ${localConnection.id}`
            : 'IPP/IPPS · Local service'}</option
        >{/if}<option value="bluetooth">Bluetooth LE</option><option value="spp">Bluetooth Serial</option><option
        value="usb">USB</option
      ></select
    ></label
  >
  {#if rollCapabilities}<fieldset>
      <legend>Continuous roll</legend>{#if rollCapabilities.automaticCutter}<label
          >Cut<select bind:value={cutMode} onchange={continuousChanged}
            >{#each availableCutModes as mode}<option value={mode}
                >{mode === 'after-each'
                  ? 'After each label'
                  : mode === 'after-job'
                    ? 'After complete job'
              ...
              >{/each}</select
          ></label
        >{/if}{#if rollCapabilities.maximumExtraFeedMm > 0}<label
          >Extra feed before (mm)<input
            type="number"
            min={rollCapabilities.minimumExtraFeedMm}
            max={rollCapabilities.maximumExtraFeedMm}
            step="0.1"
            bind:value={extraFeedBeforeMm}
            onchange={continuousChanged}
          /></label
        ><label
          >Extra feed after (mm)<input
            type="number"
            min={rollCapabilities.minimumExtraFeedMm}
            max={rollCapabilities.maximumExtraFeedMm}
            step="0.1"
            bind:value={extraFeedAfterMm}
            onchange={continuousChanged}
          /></label
        >{/if}{#if rollCapabilities.supportsChainedRaster}<label class="check"
          ><input type="checkbox" bind:checked={chainCopies} onchange={continuousChanged} /> Chain copies without intermediate
          cuts</label
        >{/if}<small
        >Required firmware feed: {rollCapabilities.requiredFeedBeforeMm ?? 0} mm before / {rollCapabilities.requiredFeedAfterMm ??
          0} mm after. Artwork margins and extra operator feed are separate. This printer currently allows {rollCapabilities.minimumLengthMm}–{rollCapabilities.maximumLengthMm}
        mm labels.</small
      >
    </fieldset>{/if}
  {#if route === 'local'}
    <p class="hint">
      {localConnection
        ? `Saved by the local service as ${localConnection.id}. It will be restored after reload.`
        : 'Pair with the local service and configure an IPP/IPPS printer.'}
    </p>
  {:else if route === 'bluetooth'}
    ...
    <details>
      <summary>BLE definition</summary><label>Service UUID<input bind:value={service} /></label><label
        >Write UUID<input bind:value={write} /></label
      ><label>Notify UUID<input bind:value={notify} /></label>
    </details>
  {:else if route === 'spp'}
    <p class="hint">
      Choose the paired serial port: COM on Windows, <code>/dev/cu.*</code> on macOS, or <code>rfcomm</code> on Linux.
    </p>
  {:else}
    <p class="hint">Choose the attached USB printer.</p>
    <details>
      <summary>USB identity</summary><label>Vendor ID<input bind:value={vendorId} placeholder="0x04f9" /></label><label
        >Product ID<input bind:value={productId} placeholder="0x20a8" /></label
      >
    </details>
  {/if}
  <div class="actions">
    {#if route === 'local'}<button onclick={onConfigureLocal}>{localConnection ? 'Manage' : 'Configure'}</button
      >{:else if connection === 'connected'}<button onclick={() => disconnect()}>Disconnect</button>{:else}<button
        class="primary"
        onclick={connect}
    ...
    <button
      ...
      disabled={(route === 'local' ? !localConnection : connection !== 'connected') || querying}
      >{querying ? 'Checking…' : 'Refresh status'}</button
    >
    <button
      class="primary print"
      onclick={print}
      disabled={continuousPrinterUnsupported ||
        (route === 'local' ? !localConnection : connection !== 'connected') ||
        !!cancellation}
      title={continuousPrinterUnsupported ? 'The selected printer is not qualified for continuous media.' : undefined}
      >{cancellation ? 'Printing…' : 'Print label'}</button
    >
  </div>
  ...
  {#if printerStatus}<dl>
      {#if printerStatus.phase}<dt>State</dt>
        ...
        <dd>{printerStatus.paper}</dd>{/if}{#if printerStatus.cover}<dt>Cover</dt>
        <dd>{printerStatus.cover}</dd>{/if}{#if printerStatus.firmware}<dt>Firmware</dt>
        <dd>{printerStatus.firmware}</dd>{/if}
      <dt>Errors</dt>
      <dd class:fault={!!printerStatus.errors.length}>
        {printerStatus.errors.length ? printerStatus.errors.join(', ') : 'none'}
      </dd>
    </dl>{/if}
</section>
