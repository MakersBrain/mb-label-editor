...
<script lang="ts">
  ...
  onMount(() => {
    ...
    void provider.fetchBlob(path).then(async blob => {
      if (!active) return;
      const trimmed = trim ? await (blob.type.includes('svg') ? trimmedSvgDataUrl(blob) : trimmedDataUrl(blob)).catch(() => '') : '';
      if (!active) return;
  ...
  function inkBounds(data: Uint8ClampedArray, width: number, height: number): { left: number; top: number; right: number; bottom: number } | null {
  ...
  async function trimmedSvgDataUrl(blob: Blob): Promise<string> {
    ...
    probe.setAttribute('viewBox', `${minX} ${minY} ${boxWidth} ${boxHeight}`); probe.setAttribute('width', String(rasterWidth)); probe.setAttribute('height', String(rasterHeight));
    const probeUrl = URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(probe)], { type: 'image/svg+xml' }));
    try {
      const image = new Image();
      await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error('svg raster failed')); image.src = probeUrl; });
      ...
      const right = Math.min(rasterWidth, bounds.right + pad + 1), bottom = Math.min(rasterHeight, bounds.bottom + pad + 1);
      if ((right - left) * (bottom - top) > rasterWidth * rasterHeight * 0.9) return '';
      const output = root.cloneNode(true) as SVGSVGElement;
      output.setAttribute('viewBox', `${minX + left / scale} ${minY + top / scale} ${(right - left) / scale} ${(bottom - top) / scale}`);
      output.removeAttribute('width'); output.removeAttribute('height'); output.setAttribute('preserveAspectRatio', 'xMidYMid meet');
