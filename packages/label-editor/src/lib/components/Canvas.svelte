...
<script lang="ts">
  ...
  function finishDrag(event: PointerEvent) { if (!drag) return; let zoom = 1; editor.view.subscribe((value) => { zoom = value.zoom; })();
    ...
    else if (drag.kind === 'resize' && drag.element) editor.execute(resizeElement(drag.element.id,{width:drag.element.transform.width+raw.x,height:drag.element.transform.height+raw.y}));
