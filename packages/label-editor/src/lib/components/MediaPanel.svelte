...
<script lang="ts">
  ...
  interface Props { editor: EditorStore; sdk?: PrinterSdk; materializer?: Pick<DocumentMaterializer, 'materializeRecord'>; printers?: PrinterDefinition[]; printerId?: string; onPrinter?: (id: string) => void }
  ...
  let loadedFor = '';
  $effect(() => { const currentSdk = sdk; const id = printerId; if (currentSdk && id && id !== loadedFor) { loadedFor = id; untrack(() => loadStock(id)).catch(() => { stock = []; }); } });
  const selectedStock = $derived(stock.find(item => Math.abs(item.widthMm - editor.document.media.width) < .01 && (item.shape === 'continuous' || Math.abs(item.heightMm - editor.document.media.height) < .01))?.id ?? '');
  ...
  const hardMinimum = $derived(selectedPrinter?.continuousMedia?.minimumLengthMm ?? selectedPrinter?.media.minHeight ?? .1);
  const hardMaximum = $derived(selectedPrinter?.continuousMedia?.maximumLengthMm ?? selectedPrinter?.media.maxHeight ?? 1000);
  ...
  $effect(() => { const document = editor.document; const currentSdk = sdk; const printer = selectedPrinter; untrack(() => calculateLength(document, currentSdk, printer)).catch(() => {}); });
  async function calculateLength(document: typeof editor.document, currentSdk: PrinterSdk | undefined, printer: PrinterDefinition | undefined){
    const generation=++calculationGeneration;
    if(document.media.shape!=='continuous'||!currentSdk?.measure){calculatedLength=undefined;calculationWarnings=[];calculationError='';return}
    const settings=continuousSettings(document);
    const limits=printer?{minimumLengthMm:printer.continuousMedia?.minimumLengthMm??printer.media.minHeight,maximumLengthMm:printer.continuousMedia?.maximumLengthMm??printer.media.maxHeight,source:'printer' as const,printerModel:printer.id}:undefined;
    try{
      const result=await prepareDocumentForOutput(document,{materializer,measurer:currentSdk as import('../continuous-media.js').DocumentMeasurer},{limits});
      if(generation===calculationGeneration){
        ...
        if(settings.lengthMode==='fixed'&&result.contentBounds&&result.contentBounds.y+result.contentBounds.height>result.resolvedLengthMm&&!warnings.some(item=>item.includes('fixed cut line')))warnings=[...warnings,'Visible content extends past the fixed cut line.'];
...
{#if printers.length}<section class="capabilities">
  <label>Printer<select value={printerId} onchange={event => onPrinter(event.currentTarget.value)}><option value="">Choose printer</option>{#each printers as printer}<option value={printer.id}>{printer.displayName} · {printer.dpi} dpi</option>{/each}</select></label>
  {#if stock.length}<label>Media for this printer<select value={selectedStock} onchange={event => stockPreset(event.currentTarget.value)}><option value="">Custom size</option>{#each stock as item}<option value={item.id}>{item.name} · {item.widthMm} × {item.heightMm || 'continuous'} mm</option>{/each}</select></label><p class="hint">Selecting stock sets the exact label size and media type used for preview and printing.</p>{/if}
</section>{/if}
<section><h2>Media & zones</h2>
  <label>Generic preset<select onchange={event => preset(event.currentTarget.value)}><option value="">Custom</option>{#each presets as item, index}<option value={index}>{item[0]}</option>{/each}</select></label>
  <div class="grid">
    <label>{editor.document.media.shape==='continuous'?'Roll width':'Width'} (mm)<input type="number" min=".1" step=".1" value={editor.document.media.width} onchange={event => media({width:+event.currentTarget.value})}></label>
    {#if editor.document.media.shape!=='continuous'}<label>Height (mm)<input type="number" min=".1" step=".1" value={editor.document.media.height} onchange={event => media({height:+event.currentTarget.value})}></label>{/if}
    <label>DPI<input type="number" min="1" step="1" value={editor.document.media.dpi} onchange={event => media({dpi:+event.currentTarget.value})}></label>
    <label>Shape<select value={editor.document.media.shape} onchange={event => media({shape:event.currentTarget.value as Media['shape']})}><option>rectangle</option><option>round</option><option>continuous</option></select></label>
  </div>
  {#if editor.document.media.shape==='continuous'}
    <fieldset class="continuous"><legend>Continuous roll</legend>
      <div class="grid">
        <label>Length mode<select value={rollSettings.lengthMode} onchange={event=>roll({lengthMode:event.currentTarget.value as typeof rollSettings.lengthMode})}><option value="fixed">Fixed length</option><option value="fit-content" disabled={!sdk?.measure}>Fit content</option></select></label>
        {#if rollSettings.lengthMode==='fixed'}<label>Cut length (mm)<input type="number" min=".1" step=".1" value={rollSettings.fixedLengthMm} onchange={event=>roll({fixedLengthMm:+event.currentTarget.value})}></label>{:else}<label>Calculated length (mm)<output>{calculatedLength===undefined?'Calculating…':calculatedLength.toFixed(2)}</output></label>{/if}
        <label>Leading safe margin (mm)<input type="number" min="0" step=".1" value={rollSettings.leadingMarginMm} onchange={event=>roll({leadingMarginMm:+event.currentTarget.value})}></label>
        <label>Trailing margin (mm)<input type="number" min="0" step=".1" value={rollSettings.trailingMarginMm} onchange={event=>roll({trailingMarginMm:+event.currentTarget.value})}></label>
        <label>Preferred minimum (mm)<input type="number" min={hardMinimum} max={hardMaximum} step=".1" value={rollSettings.preferredMinimumLengthMm??''} placeholder={String(selectedPrinter?hardMinimum:'Generic')} onchange={event=>roll({preferredMinimumLengthMm:optionalLength(event.currentTarget.value)})}></label>
        <label>Preferred maximum (mm)<input type="number" min={hardMinimum} max={hardMaximum} step=".1" value={rollSettings.preferredMaximumLengthMm??''} placeholder={String(selectedPrinter?hardMaximum:'Generic')} onchange={event=>roll({preferredMaximumLengthMm:optionalLength(event.currentTarget.value)})}></label>
        <label>Batch sizing<select value={rollSettings.batchLengthMode} onchange={event=>roll({batchLengthMode:event.currentTarget.value as typeof rollSettings.batchLengthMode})}><option value="per-record">Per record</option><option value="uniform-longest">Match longest</option></select></label>
      </div>
  ...
  {/if}
  <button onclick={() => addZone()}>Add independent zone</button><button onclick={() => addZone(true)} disabled={!editor.document.media.zones?.length}>Add cloned zone</button>{#each editor.document.media.zones ?? [] as zone}<fieldset><legend>{zone.name}</legend><div class="grid"><label>Name<input value={zone.name} onchange={event => edit(zone.id, { name: event.currentTarget.value })}></label>{#each ['x', 'y', 'width', 'height'] as key}<label>{key}<input type="number" step=".1" value={zone[key as 'x']} onchange={event => edit(zone.id, { [key]: +event.currentTarget.value })}></label>{/each}<label>Mode<select value={zone.cloneOf ?? ''} onchange={event => edit(zone.id, { cloneOf: event.currentTarget.value || undefined })}><option value="">Independent</option>{#each (editor.document.media.zones ?? []).filter(item => item.id !== zone.id) as source}<option value={source.id}>Clone {source.name}</option>{/each}</select></label></div><button onclick={() => remove(zone.id)}>Remove</button></fieldset>{/each}
</section>
