...
<nav aria-label="Drawing tools">
  <div class="group">{#each insertTypes as type}<button class="tool" onclick={()=>insertElement(editor,type)} title={`Insert ${insertLabels[type].toLowerCase()}`}><Icon name={type}/>{insertLabels[type]}</button>{/each}</div>
  <span class="spacer"></span>
  <div class="group">
    <button class="tool" onclick={()=>editor.undo()} disabled={!editor.canUndo} title={`Undo (${shortcutLabel('Mod+Z')})`}><Icon name="undo"/>Undo</button>
    <button class="tool" onclick={()=>editor.redo()} disabled={!editor.canRedo} title={`Redo (${shortcutLabel('Mod+Shift+Z')})`}><Icon name="redo"/>Redo</button>
  </div>
  <div class="group">
    <button class="tool" onclick={group} disabled={editor.selection.size<2}><Icon name="group"/>Group</button>
    <button class="tool" onclick={ungroupSelected} disabled={!editor.selectedElements.some(e=>e.type==='group')}><Icon name="ungroup"/>Ungroup</button>
  ...
  <div class="group align-tools" aria-label="Align selection inside">
    ...
    {#each alignments as item}<button class="align" title={`Align ${item[1].toLowerCase()} inside ${alignmentTarget==='root'?'label':'zone'}`} aria-label={`Align ${item[1].toLowerCase()}`} onclick={()=>align(item[0])} disabled={!editor.selection.size}>{item[1]==='Horizontal center'?'↔':item[1]==='Vertical center'?'↕':item[1]}</button>{/each}
  </div>
  <div class="group"><label class="zoom-field">Zoom<input class="zoom" type="range" min=".25" max="4" step=".25" value={editor.view.zoom} oninput={(e)=>editor.setView({zoom:+e.currentTarget.value})}><output>{Math.round(editor.view.zoom*100)}%</output></label></div>
</nav>
<style>
