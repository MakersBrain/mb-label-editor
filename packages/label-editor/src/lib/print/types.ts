...
export interface ProtocolPlan {
  ...
  totalBytes: number;
  /** Original SDK wire actions, when available. */ sdkActions?: SdkPlanAction[];
  /** SDK build that emitted the plan, echoed back when the SDK executes it. */ sdkSourceCommit?: string;
}
export type ProtocolResponseWait =
  { kind: 'response'; bytes: Uint8Array } | { kind: 'timeout' } | { kind: 'unavailable' };
...
export interface ProtocolExecutionProgress {
...
export type ProtocolPlanExecutor = (
  plan: ProtocolPlan,
  transport: ProtocolExecutionTransport,
  progress?: (state: ProtocolExecutionProgress) => void,
  signal?: AbortSignal,
...
export function adaptSdkProtocolPlan(protocol: string, actions: SdkPlanAction[], sourceCommit?: string): ProtocolPlan {
  let totalBytes = 0;
  const normalized: ProtocolAction[] = actions.map((item) => {
    switch (item.action) {
      case 'job-boundary':
        return { type: 'job-boundary', phase: item.kind, id: `${protocol}-${item.kind}` };
      case 'subscribe-notifications':
        return { type: 'subscribe', channel: 'printer' };
      case 'command-write': {
      ...
      case 'raster-write': {
        ...
        return {
          ...
          chunkable: true,
          atomic: false,
          logicalChunkSize: item.logical_chunk,
          delayAfterMs: item.delay_after_each_physical_write_ms,
        };
      }
      case 'delay':
        return { type: 'delay', milliseconds: item.milliseconds };
      case 'wait-for-response':
        return {
          type: 'wait-response',
          channel: 'printer',
          timeoutMs: item.timeout_ms,
          fallbackDelayMs: item.fallback_delay_ms,
  ...
  return {
    ...
    totalBytes,
    sdkActions: actions,
    ...(sourceCommit ? { sdkSourceCommit: sourceCommit } : {}),
  };
}
export function toSdkPlanActions(plan: ProtocolPlan): SdkPlanAction[] {
  return plan.actions.flatMap((item, index): SdkPlanAction[] => {
    switch (item.type) {
      case 'job-boundary':
        return [{ action: 'job-boundary', kind: item.phase }];
      case 'subscribe':
        return [{ action: 'subscribe-notifications' }];
      case 'write': {
        if (item.chunkable)
          return [
            {
              action: 'raster-write',
              bytes: [...item.data],
              logical_chunk: item.logicalChunkSize,
              delay_after_each_physical_write_ms: item.delayAfterMs,
            },
          ];
        const command: SdkPlanAction = {
          action: 'command-write',
          name: `editor-command-${index}`,
          bytes: [...item.data],
          atomic: item.atomic,
        };
      ...
      case 'delay':
        return [{ action: 'delay', milliseconds: item.milliseconds }];
      case 'wait-response':
        return [
          {
            action: 'wait-for-response',
            timeout_ms: item.timeoutMs,
            fallback_delay_ms: item.fallbackDelayMs ?? 0,
            validation: item.validate ?? 'any-notification',
          },
        ];
    }
  });
...
export class ContinuousPrintError extends Error {
  constructor(
    readonly code: ContinuousPrintErrorCode,
    message: string,
  ) {
    super(message);
    this.name = 'ContinuousPrintError';
  }
}
export function validateContinuousPrintOptions(
  document: LabelDocument,
  printer: PrinterDefinition,
  options: ContinuousPrintOptions | undefined,
): void {
  ...
  if (!capability?.supported)
    throw new ContinuousPrintError(
      'continuous.unsupported_printer',
      `${printer.displayName} does not support qualified continuous-media jobs.`,
    );
  if (
    !Number.isFinite(document.media.height) ||
    document.media.height < capability.minimumLengthMm ||
    document.media.height > capability.maximumLengthMm
  )
  ...
  if (!options) return;
  if (!capability.cutModes.includes(options.cutMode))
    throw new ContinuousPrintError(
      'continuous.cut_mode_unsupported',
      `Cut mode ${options.cutMode} is not supported by ${printer.displayName}.`,
    );
  for (const [label, value] of [
    ['before', options.extraFeedBeforeMm],
    ['after', options.extraFeedAfterMm],
  ] as const)
    if (!Number.isFinite(value) || value < capability.minimumExtraFeedMm || value > capability.maximumExtraFeedMm)
      throw new ContinuousPrintError(
        'continuous.feed_out_of_range',
        `Extra feed ${label} must be between ${capability.minimumExtraFeedMm} and ${capability.maximumExtraFeedMm} mm.`,
      );
  if (options.chainCopies && !capability.supportsChainedRaster)
    throw new ContinuousPrintError(
      'continuous.cut_mode_unsupported',
      `${printer.displayName} does not support chained continuous rasters.`,
    );
}
export interface PrinterDefinition {
  ...
  dpi: number;
  protocols: string[];
  media: { minWidth: number; maxWidth: number; minHeight: number; maxHeight: number };
...
export interface MediaPreset {
  ...
  name: string;
  widthMm: number;
  /** Zero for continuous stock. */ heightMm: number;
  shape: 'rectangle' | 'round' | 'continuous';
  tapeWidthMm?: number;
}
...
export interface PrinterSdk extends Partial<DocumentMeasurer> {
  ...
  importFirstPdfPage(
    data: Uint8Array,
    dpi: number,
  ): Promise<{ mimeType: 'image/png'; data: Uint8Array; widthMm: number; heightMm: number }>;
  inspectLaPoste(
    data: Uint8Array,
...
export interface PrintRequest {
  ...
  record?: number;
  continuous?: ContinuousPrintOptions;
  /** Send the raster LZO-compressed, which Phomemo firmware accepts on the M110 family. */ compressRaster?: boolean;
  signal?: AbortSignal;
