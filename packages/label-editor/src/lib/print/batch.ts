...
import { ContinuousPrintError, type BatchPrintProgress, type ContinuousPrintOptions, type PrinterDefinition, type PrintResult, type PrintRoute } from './types.js';
...
export async function executeBatch(request: BatchPrintRequest): Promise<BatchPrintResult> {
  ...
  if (request.continuous?.cutMode === 'after-job' && !nativeBatch) {
    throw new ContinuousPrintError('continuous.batch_route_unsupported','Cut after complete job requires native batch support from the selected print route.');
  }
  if (nativeBatch&&request.route.printBatch) {
    const result = await request.route.printBatch({ documents: request.documents, printer: request.printer, copies: request.copies, continuous: request.continuous, signal: request.signal, onProgress: request.onProgress });
  ...
  for (const document of request.documents) {
    ...
    const result = await request.route.print({
      document, printer: request.printer, copies: request.copies, continuous: request.continuous, signal: request.signal,
      onProgress: (current) => request.onProgress?.({ item: completed, items: request.documents.length, copy:0, copies:request.copies, current })
    });
