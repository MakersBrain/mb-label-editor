...
export class DirectPrintRoute implements PrintRoute {
  ...
  async print(request: PrintRequest): Promise<PrintResult> {
    ...
    try {
      ...
      const report = (state: ProtocolExecutionProgress) => {
        ...
        const progress = { action: state.lastCompletedAction + 1, actions: plan.actions.length, bytesSent, totalBytes: plan.totalBytes, phase: action?.type ?? 'unknown' };
      ...
      const outcome = execution.status === 'outcome-unknown' && request.signal?.aborted ? 'cancelled-partial'
        : execution.status === 'cancelled-before-send' && execution.error && !request.signal?.aborted ? 'failed' : execution.status;
  ...
  async queryStatus(printer: PrinterDefinition, options?: { signal?: AbortSignal }): Promise<PrinterStatus> {
    ...
    try {
      ...
      const result = await this.sdk.executePlan(plan, adaptTransport(transport, plan, captured), undefined, options?.signal);
...
const isResponseTimeout = (error: unknown) => error instanceof Error && ('code' in error ? (error as Error & { code?: string }).code === 'response-timeout' : /response.*timed out|no (status|notification)/i.test(error.message));
function adaptTransport(transport: DirectTransport, plan: ProtocolPlan, captured?: Uint8Array[]): ProtocolExecutionTransport {
  const payloadLimit = Math.min(transport.physicalWriteLimit ?? Infinity, transport.negotiatedAttMtu === undefined ? Infinity : transport.negotiatedAttMtu - 3);
  ...
  return {
    ...
    async subscribeNotifications(signal) { try { await transport.subscribe('printer', signal); return true; } catch (error) { if (isNotificationUnavailable(error)) return false; throw error; } },
    write: (bytes, signal) => transport.write(bytes, signal),
    async waitForResponse(timeoutMs, signal) {
      try { const bytes = await transport.waitResponse('printer', timeoutMs, validations.shift(), signal); captured?.push(bytes); return { kind: 'response', bytes }; }
      catch (error) { if (isNotificationUnavailable(error)) return { kind: 'unavailable' }; if (isResponseTimeout(error)) return { kind: 'timeout' }; throw error; }
    }
  };
}
