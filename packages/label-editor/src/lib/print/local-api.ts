...
export type LocalApiTransport =
  | { kind: 'file'; path: string }
  | { kind: 'tcp'; address: string }
  | { kind: 'serial' | 'rfcomm'; path: string; baud?: number }
  | { kind: 'ipp'; uri: string; certificatePem?: string };
export type LocalApiPrinterOperation =
  'status' | 'system-report' | 'wifi-status' | 'wifi-scan' | 'wifi-configure' | 'ipp-status';
...
export interface LocalApiConnectionStatus {
...
export interface LocalApiDiscoveryResponse {
  devices: LocalApiDiscoveryCandidate[];
...
export interface LocalApiBrotherWifiStatus {
  connectionId: string;
  status: BrotherWifiStatus;
}
export interface BrotherWifiAccessPoint {
  ssid: string;
  channel?: number | null;
  power?: number | null;
  encrypted: boolean;
...
export interface LocalApiBrotherWifiScan {
  connectionId: string;
  accessPoints: BrotherWifiAccessPoint[];
}
export interface LocalApiBrotherReport {
  connectionId: string;
  redacted: true;
...
export interface LocalApiAdminGrant {
  token: string;
  expiresAt: string;
}
export interface LocalApiOptions {
  ...
  origin?: string;
  connection?: () => LocalApiConnection | undefined;
  journal?: JobJournal;
}
export interface LocalApiJob {
  ...
  state: string;
  terminal: boolean;
  outcome?: PrintResult['outcome'] | null;
...
export class LocalApiPrintRoute implements PrintRoute {
  ...
  constructor(private options: LocalApiOptions) {
    this.baseUrl = options.baseUrl ?? 'http://127.0.0.1:9847/v1';
  }
  isSupported() {
  ...
  async negotiateCapabilities(): Promise<LocalApiCapabilities> {
    const response = await this.request('/capabilities', { headers: this.headers(false, true) });
    if (!response.ok) throw new Error(await actionableResponse(response));
    const capabilities = (await response.json()) as LocalApiCapabilities;
    this.supportsNativeBatch =
      capabilities.features.includes('native-document-batch') && capabilities.features.includes('continuous-options');
    return capabilities;
  }
  ...
  async pairAdmin(secret: string): Promise<LocalApiAdminGrant> {
    const response = await this.request('/admin/pair', {
      method: 'POST',
      headers: this.headers(true),
      body: JSON.stringify({ secret }),
    });
  ...
  async validate(document: PrintRequest['document']) {
    const response = await this.request('/documents/validate', {
      method: 'POST',
      headers: this.headers(true, true),
      body: JSON.stringify(toSdkDocument(document)),
    });
    if (!response.ok) throw new Error(await actionableResponse(response));
  ...
  async configureConnection(
    connection: Pick<LocalApiConnection, 'id' | 'model' | 'transport'>,
  ): Promise<LocalApiConnection> {
  ...
  async connectionStatus(id: string): Promise<LocalApiConnectionStatus> {
    const response = await this.request(`/status?connection=${encodeURIComponent(id)}`, {
  ...
  async brotherWifiScan(connectionId: string): Promise<LocalApiBrotherWifiScan> {
    const id = encodeURIComponent(connectionId);
    const response = await this.request(`/printers/${id}/brother/wifi/scan`, {
      method: 'POST',
      headers: this.headers(false, true),
    });
    if (!response.ok) throw new Error(await actionableResponse(response));
   ...
   */
  async prepareBrotherWifiConfigure(
    connectionId: string,
    request: BrotherWifiConfigureRequest,
    adminToken: string,
  ): Promise<LocalApiBrotherWifiConfigurePreparation> {
    const id = encodeURIComponent(connectionId);
    ...
    if (!response.ok) throw new Error(await actionableResponse(response));
    const wire = (await response.json()) as Omit<LocalApiBrotherWifiConfigurePreparation, 'summary'> &
      BrotherWifiConfigureSummary;
    const {
      connection,
      device,
      ssid,
  ...
  async configureBrotherWifi(
    connectionId: string,
    approvalId: string,
    request: BrotherWifiConfigureRequest,
    adminToken: string,
  ): Promise<LocalApiBrotherWifiConfigureResult> {
    const id = encodeURIComponent(connectionId);
    const response = await this.request(`/printers/${id}/brother/wifi/configure`, {
      method: 'POST',
      headers: this.adminHeaders(adminToken),
      body: JSON.stringify({ approvalId, ...request }),
    });
    if (!response.ok) throw new Error(await actionableResponse(response));
  ...
  private prepare(request: PrintRequest) {
    ...
    if (!connection)
      throw new Error(
        'Select a persisted, probed local-service printer connection before printing. Capture is never a printing destination.',
      );
    if (
      ['unavailable', 'error'].includes(connection.status) ||
      !['file', 'tcp', 'serial', 'rfcomm', 'ipp'].includes(connection.transport.kind)
    )
      throw new Error('The selected local-service connection is not a ready physical transport.');
    const idempotencyKey =
      globalThis.crypto?.randomUUID?.() ?? `mb-editor-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  private async submitPrepared(
    ...
    const response = await this.request('/jobs', {
      method: 'POST',
      signal,
      headers: { ...this.headers(true, true), 'idempotency-key': idempotencyKey },
      body: requestBody,
    });
  ...
  async cancel(id: string, signal?: AbortSignal) {
    const response = await this.request(`/jobs/${encodeURIComponent(id)}/cancel`, {
      method: 'POST',
      signal,
      headers: this.headers(false, true),
    });
  ...
  async events(id: string, onProgress: (progress: PrintProgress) => void, signal?: AbortSignal) {
    const response = await this.request(`/jobs/${encodeURIComponent(id)}/events`, {
      signal,
      headers: { ...this.headers(false, true), accept: 'text/event-stream' },
    });
    if (!response.ok) throw new Error(await actionableResponse(response));
    ...
    let buffered = '';
    let terminal: PrintResult | undefined;
    for (;;) {
      ...
      for (const block of blocks) {
        const data = block
          ...
          .join('\n');
        if (!data) continue;
        const job = JSON.parse(data) as LocalApiJob;
        onProgress(progressOf(job));
        if (job.terminal) terminal = normalizeJob(job);
      }
      if (done) break;
      if (terminal) {
        await reader.cancel();
  ...
  async print(request: PrintRequest): Promise<PrintResult> {
    if (!this.options.token())
      return {
        ...
        error: 'Pair with the local printer service first.',
      };
    if (request.continuous?.cutMode === 'after-job' && !this.supportsNativeBatch)
      return {
        ...
        bytesSent: 0,
        lastCompletedAction: -1,
        error: 'Cut after job requires negotiated native batch support from the local printer service.',
      };
    const prepared = this.prepare(request);
  ...
  async printBatch(request: Parameters<NonNullable<PrintRoute['printBatch']>>[0]): Promise<PrintResult> {
    ...
    if (!this.supportsNativeBatch)
      return {
        ...
        bytesSent: 0,
        lastCompletedAction: -1,
        error: 'The local printer service has not negotiated native batch support. Update it and reconnect.',
      };
    const prepared = this.prepareBatch(request);
    return await this.printPrepared(request.documents[0]?.id ?? 'batch', prepared, request.signal, undefined, (job) =>
      request.onProgress?.({
        item: job.item ?? 0,
        items: job.items ?? request.documents.length,
        copy: job.copy ?? 0,
        copies: job.copies ?? request.copies,
        current: progressOf(job),
      }),
    );
  }

  private async printPrepared(
    documentId: string,
    prepared: { idempotencyKey: string; requestBody: string },
    signal?: AbortSignal,
    onProgress?: (progress: PrintProgress) => void,
    onJob?: (job: LocalApiJob) => void,
  ): Promise<PrintResult> {
    let persisted = await this.options.journal?.save({
      id: `local:${prepared.idempotencyKey}`,
      documentId,
    ...
    try {
      ...
      const terminal = submitted.terminal
        ? submitted
        : await this.poll(submitted.id, signal, onProgress, async (job) => {
            onJob?.(job);
            if (persisted)
              persisted = await this.save(persisted, job.state, true, { ...localDetails(persisted), lastJob: job });
          });
      const result = normalizeJob(terminal);
      if (persisted)
        await this.save(persisted, terminal.state, isAmbiguous(result), {
          ...localDetails(persisted),
          lastJob: terminal,
        });
      return result;
    } catch (error) {
      const uncertain =
        !(error instanceof LocalApiResponseError) || !!(persisted && localDetails(persisted).remoteJobId);
      const details: LocalApiJobDetails = persisted
        ? localDetails(persisted)
        : { kind: 'local-api-print', ...prepared };
      if (persisted)
        await this.save(
          persisted,
          uncertain ? 'status-unknown' : 'failed',
          uncertain,
          uncertain ? details : { ...details, requestBody: undefined },
        );
      return uncertain
        ? {
            ...
            error: 'The local job may have been accepted. Resume its status before printing again.',
          }
        : {
            outcome: 'failed',
            bytesSent: 0,
  ...
  async recover(
    ...
    try {
      ...
      const terminal = remote.terminal
        ? remote
        : await this.poll(remote.id, signal, onProgress, async (current) => {
  ...
  private requireToken() {
    ...
    if (!token) throw new Error('Pair with the local printer service first.');
    return token;
  }
}
...
const progressOf = (job: LocalApiJob): PrintProgress => ({
  action: job.action,
  actions: job.actions,
...
const normalizeJob = (job: LocalApiJob): PrintResult => ({
  outcome: job.outcome ?? (job.terminal ? 'failed' : 'outcome-unknown'),
  lastCompletedAction: job.lastCompletedAction,
  bytesSent: job.bytesSent,
  error: job.error ?? undefined,
});
const localDetails = (job: PersistedJob | undefined): LocalApiJobDetails => {
  if (!job || !isLocalApiJobDetails(job.details)) throw new Error('Invalid local job recovery details.');
  return job.details;
};
...
const abortableDelay = (milliseconds: number, signal?: AbortSignal) =>
  new Promise<void>((resolve, reject) => {
    const aborted = () => {
      ...
      reject(signal?.reason ?? new DOMException('Stopped watching local job.', 'AbortError'));
    };
...
async function actionableResponse(response: Response) {
  if (response.status === 401)
    return 'Pairing secret expired or the saved grant was revoked. Generate a fresh one-time secret and pair with the local service again.';
  if (response.status === 403)
    return 'The local service denied this editor origin. Add the exact editor origin to LABEL_EDITOR_ORIGINS, restart mb-printer api, and pair again.';
  if (response.status === 413) return 'The document or job is too large for the local service.';
