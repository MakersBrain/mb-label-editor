...
export class CloudPrintJobController {
  ...
  constructor(readonly client: CloudPrintClient, readonly pollIntervalMs = 1000) {}
  get current() { return this.#current; }
  subscribe(listener: (job: CloudPrintJob | undefined) => void) { this.#listeners.add(listener); listener(this.#current); return () => this.#listeners.delete(listener); }
  publish(job: CloudPrintJob | undefined) { this.#current = job; for (const listener of this.#listeners) listener(job); }
  ...
  async resume(jobId: string, signal?: AbortSignal, onProgress?: (job: CloudPrintJob) => void) { return await this.poll(jobId, signal, onProgress); }
  async poll(jobId: string, signal?: AbortSignal, onProgress?: (job: CloudPrintJob) => void): Promise<CloudPrintJob> {
    for (;;) {
      if (signal?.aborted) throw signal.reason ?? new DOMException('Aborted', 'AbortError');
      const job = await this.client.getJob(jobId, signal); this.publish(job); onProgress?.(job);
...
export class CloudPrintRoute implements PrintRoute {
  ...
  async print(request: PrintRequest): Promise<PrintResult> {
    ...
    this.#active = true;
    try { return await this.printOnce(request); }
  ...
  private async printOnce(request: PrintRequest): Promise<PrintResult> {
    ...
    let persisted = await this.begin(request.document, {
      kind: 'cloud-print', idempotencyKey, printerId: printer.id, model: printer.model,
      copies: request.copies, density: request.density, submission: serialized
    });
    try {
      ...
      const terminal = isTerminal(submitted) ? submitted : await this.controller.poll(submitted.id, request.signal, job => {
      ...
      await this.save(persisted, uncertain ? 'status-unknown' : 'failed', uncertain, uncertain ? details : { ...details, submission: undefined });
      return uncertain ? { outcome: 'outcome-unknown', lastCompletedAction: details.lastJob?.lastCompletedAction ?? -1, bytesSent: details.lastJob?.bytesSent ?? 0, error: 'The cloud job may still be queued or printing. Resume its status before printing again.' } : failed(error instanceof Error ? error.message : String(error));
    }
  }
  ...
  private async begin(document: LabelDocument, details: CloudJobDetails) {
    return await this.options.journal.save({ id: uuid(), documentId: document.id, createdAt: new Date().toISOString(), state: 'submitting', route: this.id, resumable: true, details });
  }
...
export function isCloudJobDetails(value: unknown): value is CloudJobDetails { return !!value && typeof value === 'object' && (value as { kind?: unknown }).kind === 'cloud-print'; }
const detailsOf = (job: PersistedJob): CloudJobDetails => { if (!isCloudJobDetails(job.details)) throw new Error('Invalid cloud job recovery details.'); return job.details; };
const isTerminal = (job: CloudPrintJob) => TERMINAL.has(job.terminalOutcome ?? job.state);
const ambiguous = (job: CloudPrintJob) => ['cancelled-partial', 'outcome-unknown'].includes(job.terminalOutcome ?? job.state);
const progressOf = (job: CloudPrintJob): PrintProgress => ({ action: Math.max(0, job.lastCompletedAction + 1), actions: job.actionCount, bytesSent: job.bytesSent, totalBytes: job.totalBytes, phase: job.action ?? job.state });
const resultOf = (job: CloudPrintJob): PrintResult => { const value = job.terminalOutcome ?? job.state; const outcome = OUTCOMES.has(value as PrintResult['outcome']) ? value as PrintResult['outcome'] : 'failed'; return { outcome, lastCompletedAction: job.lastCompletedAction, bytesSent: job.bytesSent, ...(job.errorCode ? { error: job.errorCode } : {}) }; };
...
const abortableDelay = (milliseconds: number, signal?: AbortSignal) => new Promise<void>((resolve, reject) => {
  const aborted = () => { clearTimeout(timeout); reject(signal?.reason ?? new DOMException('Aborted', 'AbortError')); };
