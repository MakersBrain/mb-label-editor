...
import { fromSdkDocument, toSdkDocument } from './sdk-document.js';
...
export const serializeDocument = (document: LabelDocument) => {
  if (
    ...
    continuousSettings(document).lengthMode === 'fit-content' &&
    !isResolvedLabelDocument(document)
  )
    throw new ContinuousMediaError(
      'continuous.unresolved_document',
      'Fit-content continuous labels must be prepared before serialization.',
    );
  return JSON.stringify(toSdkDocument(document), null, 2) + '\n';
};
export function parseDocument(text: string): LabelDocument {
  return fromSdkDocument(JSON.parse(text) as unknown);
}
...
export async function openDocument(
  file: File | undefined,
  validator: CanonicalDocumentValidator,
): Promise<{ document: LabelDocument; handle?: FileSystemFileHandle }> {
  let handle: FileSystemFileHandle | undefined;
  if (!file && typeof window !== 'undefined' && (window as PickerWindow).showOpenFilePicker)
    [handle] = await (window as PickerWindow).showOpenFilePicker!({ types: labelFileTypes, multiple: false });
  const selected = file ?? (await handle?.getFile());
...
export async function saveDocument(
  ...
  suggestedName = `${document.title}.mb-label.json`,
  existing?: FileSystemFileHandle,
) {
  const contents = serializeDocument(document);
  let handle = existing;
  if (!handle && typeof window !== 'undefined' && (window as PickerWindow).showSaveFilePicker)
    handle = await (window as PickerWindow).showSaveFilePicker!({ suggestedName, types: labelFileTypes });
  if (handle) {
