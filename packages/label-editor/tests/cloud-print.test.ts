...
const definition: PrinterDefinition = {
  ...
  dpi: 203,
  protocols: ['phomemo'],
  media: { minWidth: 10, maxWidth: 50, minHeight: 10, maxHeight: 100 },
};
const job = (state: string, terminalOutcome: string | null = null): CloudPrintJob => ({
  ...
  progress: null,
  action: state,
  bytesSent: state === 'completed' ? 42 : 0,
  totalBytes: 42,
  lastCompletedAction: state === 'completed' ? 3 : -1,
  actionCount: 4,
  item: 0,
  items: 1,
  copy: 0,
...
describe('cloud print OpenAPI client and route', () => {
  it('uses the current token and tenant-scoped generated paths', async () => {
    ...
    const client = new CloudPrintClient({
  ...
  it('submits canonical v4 without transport fields and maps terminal progress', async () => {
    ...
    const route = new CloudPrintRoute({
      client,
      printer: () => printer,
    ...
    const result = await route.print({
  ...
  it('reuses the byte-identical snapshot after an uncertain POST', async () => {
    ...
    const route = new CloudPrintRoute({
      ...
      printer: () => printer,
      journal: new JobJournal(database),
      controller: new CloudPrintJobController(client, 0),
    });
  ...
  it('publishes cancellation and keeps polling for its authoritative outcome', async () => {
    ...
    controller.subscribe((value) => {
      if (value) published.push(value.terminalOutcome ?? value.state);
    });
  ...
  it('submits multiple documents in one cloud job snapshot', async () => {
    ...
    const fetcher = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (request.url.endsWith('/openapi.json'))
        return response({
          components: { schemas: { ValidatedPrintRequest: { properties: { documents: {}, continuous: {} } } } },
        });
      if (request.method === 'POST') {
        bodies.push(await request.text());
        return response(job('completed'), 202);
    ...
    const route = new CloudPrintRoute({
      client,
      printer: () => printer,
      journal: new JobJournal(new EditorDatabase()),
      controller: new CloudPrintJobController(client, 0),
    });
    ...
    const wire = JSON.parse(bodies[0]);
    expect(wire.request).not.toHaveProperty('document');
    expect(wire.request.documents.map((document: { media: { height: number } }) => document.media.height)).toEqual([
      30_000, 45_000,
    ]);
  });
});
