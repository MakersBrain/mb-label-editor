  ...
  items: [
    {
      ...
      previewUrl: '/v1/assets/asset-1/preview',
    },
  ],
  total: 1,
  page: 1,
...
describe('asset catalog OpenAPI client', () => {
  it('serializes repeated filters and attaches the current bearer token', async () => {
    ...
    const fetcher = vi.fn(
      async (_input: RequestInfo | URL, _init?: RequestInit) =>
        new Response(JSON.stringify(assetPage), { status: 200, headers: { 'content-type': 'application/json' } }),
    );
    const client = new AssetCatalogClient({ baseUrl: 'http://127.0.0.1:8766/', token: () => token, fetch: fetcher });
    const page = await client.searchAssets({
      query: 'rabbit',
      providers: ['brother', 'printmaster'],
      categories: ['icon'],
      pageSize: 12,
    });
    ...
    expect(new Request(fetcher.mock.calls[1][0], fetcher.mock.calls[1][1]).headers.get('authorization')).toBe(
      'Bearer second',
    );
  });
  ...
  it('turns API failures into actionable errors', async () => {
    const fetcher = vi.fn(
      async (_input: RequestInfo | URL, _init?: RequestInit) =>
        new Response(JSON.stringify({ detail: 'missing or invalid bearer token' }), {
          status: 401,
