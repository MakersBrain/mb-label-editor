...
import { isSheetLayoutPreset, isSheetPlan, isSheetPlanForRequest, type SheetDefinition, type SheetLayoutPreset } from '../src/lib/sheets/types.js';
...
describe('classic sheet domain', () => {
  it('normalizes physical dimensions to exact integer micrometres', () => {
    ...
    expect(presetToDefinition(preset)).toMatchObject({
      kind: 'grid', paperWidthUm: 210_000, paperHeightUm: 297_000,
      labelWidthUm: 70_000, labelHeightUm: 36_000, marginTopUm: 4_500
    });
  });
  ...
  it('returns isolated preset copies and rejects layouts outside the paper', () => {
    ...
    const explicit: SheetLayoutPreset = { id: 'outside', name: 'Outside', paper: 'a4', orientation: 'portrait', slots: [{ xMm: 209, yMm: 0, widthMm: 2, heightMm: 1 }] };
  ...
  it('guards the sheet-plan boundary at runtime', () => {
    expect(isSheetPlan({ pageCount: 1, layout: { id: 'x', paperWidthUm: 1, paperHeightUm: 1, slots: [{ xUm: 0, yUm: 0, widthUm: 1, heightUm: 1 }] }, placements: [{ item: 0, page: 0, slot: 0 }] })).toBe(true);
    expect(isSheetPlan({ pageCount: 1, layout: { id: 'x', paperWidthUm: 1, paperHeightUm: 1, slots: [] }, placements: [{ item: -1, page: 0, slot: 0 }] })).toBe(false);
    expect(isSheetPlan({ pageCount: 1, layout: { id: 'x', paperWidthUm: 10, paperHeightUm: 10, slots: [{ xUm: 9, yUm: 0, widthUm: 2, heightUm: 1 }] }, placements: [{ item: 0, page: 0, slot: 0 }] })).toBe(false);
    expect(isSheetPlan({ pageCount: 1, layout: { id: 'x', paperWidthUm: 10, paperHeightUm: 10, slots: [{ xUm: 0, yUm: 0, widthUm: 1, heightUm: 1 }] }, placements: [{ item: 0, page: 0, slot: 1 }] })).toBe(false);
    expect(isSheetPlan({ pageCount: 1, layout: { id: 'x', paperWidthUm: 10, paperHeightUm: 10, slots: [{ xUm: 0, yUm: 0, widthUm: 1, heightUm: 1 }] }, placements: [{ item: 0, page: 0, slot: 0 }, { item: 0, page: 0, slot: 0 }] })).toBe(false);
  });

  it('rejects valid-looking plans that do not match the exact request', () => {
    const layout: SheetDefinition = { kind: 'explicit', id: 'two', paperWidthUm: 20, paperHeightUm: 10, slots: [{ xUm: 0, yUm: 0, widthUm: 10, heightUm: 10 }, { xUm: 10, yUm: 0, widthUm: 10, heightUm: 10 }] };
    const input = { itemCount: 2, labelWidthUm: 10, labelHeightUm: 10 };
    const options = { firstSlot: 1, dpi: 300 };
    const valid = { pageCount: 2, layout: { id: layout.id, paperWidthUm: 20, paperHeightUm: 10, slots: layout.slots }, placements: [{ item: 0, page: 0, slot: 1 }, { item: 1, page: 1, slot: 0 }] };
    expect(isSheetPlanForRequest(valid, input, layout, options)).toBe(true);
    expect(isSheetPlanForRequest({ ...valid, placements: [{ item: 0, page: 0, slot: 0 }, valid.placements[1]] }, input, layout, options)).toBe(false);
  ...
  it('validates preset wire shapes without trusting TypeScript casts', () => {
    expect(isSheetLayoutPreset({ id: 'x', name: 'X', paper: 'a4', orientation: 'portrait', grid: { rows: 1, columns: 1, labelWidthMm: 10, labelHeightMm: 10, marginLeftMm: 0, marginTopMm: 0, gapXMm: 0, gapYMm: 0, fillOrder: 'row-major' } })).toBe(true);
    expect(isSheetLayoutPreset({ id: 'x', name: 'X', paper: 'a4', orientation: 'portrait', grid: { rows: 1.5 } })).toBe(false);
