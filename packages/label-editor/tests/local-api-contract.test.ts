...
it('exchanges an administrator pairing secret without using the stored print grant',async()=>{const fetch=vi.fn(async(url:string,init?:RequestInit)=>{expect(url).toBe('http://127.0.0.1:9847/v1/admin/pair');expect(new Headers(init?.headers).get('authorization')).toBeNull();expect(JSON.parse(String(init?.body))).toEqual({secret:'admin-once'});return new Response(JSON.stringify({token:'admin-token',expiresAt:'2026-09-01T00:10:00Z'}),{status:200,headers:{'content-type':'application/json'}})});vi.stubGlobal('fetch',fetch);const grant=await new LocalApiPrintRoute({token:()=> 'print-token'}).pairAdmin('admin-once');expect(grant).toEqual({token:'admin-token',expiresAt:'2026-09-01T00:10:00Z'});expect(fetch.mock.calls[0][1]).toMatchObject({cache:'no-store'})});
it('uses the documented Brother Wi-Fi review and configure wire forms',async()=>{const request={ssid:'Workshop',password:'not-retained',encryption:'aes',authentication:'wpa2-only',infrastructure:true,wirelessDirect:false,reboot:true};let call=0;const fetch=vi.fn(async(url:string,init?:RequestInit)=>{call++;expect(new Headers(init?.headers).get('authorization')).toBe('Bearer admin-token');if(call===1){expect(url).toBe('http://127.0.0.1:9847/v1/printers/desk/brother/wifi/prepare');expect(JSON.parse(String(init?.body))).toEqual(request);return new Response(JSON.stringify({approvalId:'approval-1',expiresAt:1788250200,connection:'desk',device:'usb-device:04f9:209b:1:2',ssid:'Workshop',encryption:'aes',authentication:'wpa2-only',infrastructure:true,wirelessDirect:false,reboot:true,recovery:'Keep USB available.'}))}expect(url).toBe('http://127.0.0.1:9847/v1/printers/desk/brother/wifi/configure');expect(JSON.parse(String(init?.body))).toEqual({approvalId:'approval-1',...request});return new Response(JSON.stringify({connection:'desk',device:'usb-device:04f9:209b:1:2',applied:true,reboot:true}))});vi.stubGlobal('fetch',fetch);const route=new LocalApiPrintRoute({token:()=> 'print-token'});const preparation=await route.prepareBrotherWifiConfigure('desk',request,'admin-token');expect(preparation).toEqual({approvalId:'approval-1',expiresAt:1788250200,recovery:'Keep USB available.',summary:{connection:'desk',device:'usb-device:04f9:209b:1:2',ssid:'Workshop',encryption:'aes',authentication:'wpa2-only',infrastructure:true,wirelessDirect:false,reboot:true}});expect(await route.configureBrotherWifi('desk',preparation.approvalId,request,'admin-token')).toEqual({connection:'desk',device:'usb-device:04f9:209b:1:2',applied:true,reboot:true});for(const[,init]of fetch.mock.calls)expect(init).toMatchObject({cache:'no-store'})});
