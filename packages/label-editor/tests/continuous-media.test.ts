...
const measurement = (bottom: number, top = 3): DocumentMeasurement => ({
  ...
  elements: [{ instanceId: 'item', sourceElementId: 'item', bounds: { x: 2, y: top, width: 10, height: bottom - top } }],
});
...
describe('continuous media resolution', () => {
  it('resolves measured content without mutating the authored document', () => {
    ...
    const result = resolveContinuousDocument(source, measurement(20), { minimumLengthMm: 10, maximumLengthMm: 100, source: 'printer', printerModel: 'test' });
    ...
    expect(resolutionStamp(result.document)).toMatchObject({ layoutVersion: 'test-layout-v1', qualifiedPrinterModel: 'test' });
  ...
  it('preserves printable top and bottom insets', () => {
    ...
    const result = resolveContinuousDocument(source, measurement(20), { minimumLengthMm: 10, maximumLengthMm: 100, source: 'printer' });
  ...
  it('never clamps oversized content down to the maximum', () => {
    const source = continuous();
    expect(() => resolveContinuousDocument(source, measurement(99), { minimumLengthMm: 10, maximumLengthMm: 100, source: 'printer' }))
      .toThrowError(expect.objectContaining({ code: 'continuous.content_exceeds_maximum' } satisfies Partial<ContinuousMediaError>));
  });
  ...
  it('uses the effective preferred range and minimum for empty content', () => {
    ...
    const result = resolveContinuousDocument(source, { layoutVersion: 'test', elements: [] }, { minimumLengthMm: 10, maximumLengthMm: 100, source: 'printer' });
  ...
  it('warns instead of expanding fixed labels whose content crosses the cut line', () => {
    ...
    const result = resolveContinuousDocument(source, measurement(35), { minimumLengthMm: 10, maximumLengthMm: 100, source: 'printer' });
  ...
  it('resolves a batch per record or to the longest result', () => {
    ...
    expect(resolveContinuousBatch([first, second], [measurement(20), measurement(30)], 'per-record', limits).map((item) => item.resolvedLengthMm)).toEqual([23, 33]);
    const uniform = resolveContinuousBatch([first, second], [measurement(20), measurement(30)], 'uniform-longest', limits);
  ...
  it('keeps canonical Y as feed direction in portrait and landscape',()=>{
    const lengths=(['portrait','landscape'] as const).map(orientation=>{const source=continuous();source.media.orientation=orientation;return resolveContinuousDocument(source,measurement(20),{minimumLengthMm:10,maximumLengthMm:100,source:'printer'}).resolvedLengthMm});
    expect(lengths).toEqual([23,23]);
  });

  it('rounds resolved millimetres to the canonical micrometre grid',()=>{
    const source=continuous();
    const result=resolveContinuousDocument(source,measurement(20.1236),{minimumLengthMm:10,maximumLengthMm:100,source:'printer'});
  ...
  it('exposes authored validation without requiring a measurement',()=>{
    ...
    expect(()=>validateContinuousMedia(source)).toThrowError(expect.objectContaining({code:'continuous.invalid_width'}));
  });
});
...
describe('continuous settings serialization', () => {
  it('keeps editor settings in metadata while canonical media stays strict', () => {
    ...
    expect((canonical.extensions['makersbrain.editor:state'] as { continuousSettings?: unknown }).continuousSettings).toEqual(source.media.continuousSettings);
  ...
  it('rejects unresolved fit-content JSON and serializes a prepared document', () => {
    const source = continuous();
    expect(() => serializeDocument(source)).toThrowError(expect.objectContaining({ code: 'continuous.unresolved_document' }));
  ...
  it('resolves from the current record while preserving template authoring data for JSON', async () => {
    ...
    source.elements = [{ id: 'text', name: '{{name}}', type: 'text', text: '{{name}}', fontFamily: 'sans-serif', fontSize: 3, fontWeight: 400, horizontalAlign: 'left', verticalAlign: 'top', overflow: 'auto-height', transform: { x: 0, y: 0, width: 20, height: 5, rotation: 0 }, zIndex: 0, visible: true, locked: false }];
    let measuredText = '';
    const prepared = await prepareDocumentForOutput(source, {
      materializer: { materializeRecord: async (document, record) => { const copy = structuredClone(document); const text = copy.elements[0]; if (text?.type === 'text') text.text = record.name; return copy; } },
      measurer: { measure: async (document) => { const text = document.elements[0]; measuredText = text?.type === 'text' ? text.text : ''; return measurement(25); } },
