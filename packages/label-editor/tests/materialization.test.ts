...
} from '../src/lib/materialization.js';
...
describe('materialization boundary', () => {
  it('accepts only the exact contiguous Rust placement for the request', () => {
    ...
    const plan = {
      pageCount: 2,
      placements: [
        { record: 0, page: 0, zone: 'left' },
        { record: 1, page: 0, zone: 'right' },
        { record: 2, page: 1, zone: 'left' },
      ],
    };
    expect(isZoneBatchPlanForRequest(plan, input)).toBe(true);
    expect(
      isZoneBatchPlanForRequest(
        { ...plan, placements: [plan.placements[0], plan.placements[0], plan.placements[2]] },
        input,
      ),
    ).toBe(false);
    expect(
      isZoneBatchPlanForRequest({ ...plan, placements: plan.placements.map((item) => ({ ...item, page: 2 })) }, input),
    ).toBe(false);
  ...
  it('allowlists wasm failure fields and replaces raw messages', () => {
    const error = structuredMaterializationError({
      version: 1,
      code: 'batch.unknown_zone',
      message: 'secret document value',
      details: { index: 2, secret: 9 },
    });
