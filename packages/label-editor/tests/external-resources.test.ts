...
describe('external resource connection manager', () => {
  it('persists provider-neutral metadata but keeps credentials in memory', async () => {
    ...
    const connection = manager.upsert({ name: 'Workshop library', providerKind: adapter.value.kind, endpoint: 'https://assets.example.test/' });
    ...
    expect(manager.connections()[0]).toEqual({ version: 1, id: connection.id, name: 'Workshop library', providerKind: 'future-provider', endpoint: 'https://assets.example.test', enabled: true });
  });
  ...
  it('supports later provider factories without changing the manager', () => {
    ...
    const connection = manager.upsert({ name: 'Later service', providerKind: 'later', endpoint: 'https://later.example.test/api' });
    expect(manager.provider(connection.id)?.kind).toBe('later');
    expect(manager.providers()).toEqual([{ kind: 'first', displayName: 'Future provider' }, { kind: 'later', displayName: 'Future provider' }]);
  });

  it('drops malformed stored connections and rejects unsafe endpoints', () => {
    const adapter = factory();
    const manager = new ExternalResourceConnectionManager([adapter.value], [{ version: 1, id: 'bad', name: 'Bad', providerKind: adapter.value.kind, endpoint: 'javascript:alert(1)', enabled: true }]);
    expect(manager.connections()).toEqual([]);
    expect(() => manager.upsert({ name: 'Bad', providerKind: adapter.value.kind, endpoint: 'file:///tmp/assets' })).toThrow(/HTTP or HTTPS/);
  });
});
